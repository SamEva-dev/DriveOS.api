...
public static class AssessmentAppointmentEndpoints
{
    ...
    {
        RouteGroupBuilder group = endpoints.MapGroup("/api/crm").WithTags("CRM - Assessments");

        group
            .MapGet("/leads/{leadId:guid}/assessments", GetByLeadAsync)
            .RequireAuthorization("Crm.Assessments.Read");
        group
            .MapGet("/assessments/{appointmentId:guid}", GetByIdAsync)
            .RequireAuthorization("Crm.Assessments.Read");
        group
            .MapPost("/leads/{leadId:guid}/assessments", ScheduleAsync)
            .RequireAuthorization("Crm.Assessments.Schedule", "Branches.Read");
        group
            .MapPut("/assessments/{appointmentId:guid}/schedule", RescheduleAsync)
            .RequireAuthorization("Crm.Assessments.Schedule");
        group
            .MapPost("/assessments/{appointmentId:guid}/cancel", CancelAsync)
            .RequireAuthorization("Crm.Assessments.Cancel");
        group
            .MapPost("/assessments/{appointmentId:guid}/session/start", StartSessionAsync)
            .RequireAuthorization("Crm.Assessments.Start");
        group
            .MapGet("/assessments/{appointmentId:guid}/session", GetSessionAsync)
            .RequireAuthorization("Crm.Assessments.Read");
        group
            .MapPut("/assessments/{appointmentId:guid}/session/draft", SaveDraftAsync)
            .RequireAuthorization("Crm.Assessments.Complete", "Crm.AssessmentNotes.Create");
        group
            .MapPost("/assessments/{appointmentId:guid}/session/submit", SubmitSessionAsync)
            .RequireAuthorization("Crm.Assessments.Submit");
        group
            .MapPut("/assessments/{appointmentId:guid}/result", SaveResultAsync)
            .RequireAuthorization("Crm.Assessments.Result.Create");
        group
            .MapGet("/assessments/{appointmentId:guid}/result", GetResultAsync)
            .RequireAuthorization("Crm.Assessments.Result.Read");
        group
            .MapPost(
                "/assessments/{appointmentId:guid}/result/request-correction",
                RequestResultCorrectionAsync
            )
            .RequireAuthorization("Crm.Assessments.Result.Validate");
        group
            .MapPost("/assessments/{appointmentId:guid}/result/validate", ValidateResultAsync)
            .RequireAuthorization("Crm.Assessments.Result.Validate");
        group
            .MapPost("/assessments/{appointmentId:guid}/result/share", ShareResultAsync)
    ...
    )
    {
        ...
        BranchId? branchId = request.BranchId is null ? null : new BranchId(request.BranchId.Value);

        if (branchId.HasValue)
        {
            bool branchAvailable = await organizationsDbContext
        ...
        return result.IsFailure
            ...
            : Results.Created(
                $"/api/crm/assessments/{result.Value}",
                new { appointmentId = result.Value }
            );
    }
    ...
    )
    {
        ...
        Result<Guid> result = await mediator.Send(
            new StartAssessmentCommand(
                ...
                request.QuestionnaireCode,
                request.QuestionnaireVersion,
                request.QuestionnaireSnapshot.GetRawText(),
                DateTimeOffset.UtcNow
            ),
            ct
        );
        return result.IsFailure
            ? result.Error.ToHttpResult(context)
            : Results.Created(
                $"/api/crm/assessments/{appointmentId}/session",
                new { sessionId = result.Value }
            );
    }
    ...
    )
    {
        ...
        Result result = await mediator.Send(
            new SaveAssessmentDraftCommand(
                ...
                request.FactualObservations,
                request.PedagogicalInterpretation,
                request.Recommendation,
                request.InternalNotes,
                request.ProspectComment,
                request.DraftCompleted,
                DateTimeOffset.UtcNow
            ),
            ct
        );
    ...
    private static async Task<IResult> SubmitSessionAsync(
        Guid appointmentId,
        IMediator mediator,
        ICurrentTenant tenant,
        ICurrentUser currentUser,
    ...
    )
    {
        ...
        if (!currentUser.IsAuthenticated || currentUser.UserId is null)
            return LeadErrors.CurrentUserRequired.ToHttpResult(context);
        Result result = await mediator.Send(
            new SaveAssessmentResultCommand(
                tenant.OrganizationId.Value,
                new AssessmentAppointmentId(appointmentId),
                currentUser.UserId.Value,
                request.ExpectedRevision,
                request.Result.GetRawText(),
                request.Confidence,
                request.AiSuggestion?.GetRawText(),
                DateTimeOffset.UtcNow
            ),
            ct
        );
    ...
    )
    {
        ...
        return Results.Ok(
            new AssessmentResultResponse(
                ...
                session.ResultConfidence,
                session.ResultStatus,
                session.CorrectionReason,
                session.InternalNotes,
                session.ResultValidatedAtUtc,
                session.ResultValidatedByUserId,
                session.ResultSharedAtUtc,
                session.ResultSharedByUserId
            )
        );
    }

    private static async Task<IResult> RequestResultCorrectionAsync(
        Guid appointmentId,
        RequestAssessmentResultCorrectionRequest request,
        IMediator mediator,
        ICurrentTenant tenant,
        ICurrentUser currentUser,
        HttpContext context,
    ...
    )
    {
        ...
        if (!currentUser.IsAuthenticated || currentUser.UserId is null)
            return LeadErrors.CurrentUserRequired.ToHttpResult(context);
        Result result = await mediator.Send(
            new RequestAssessmentResultCorrectionCommand(
                tenant.OrganizationId.Value,
                new AssessmentAppointmentId(appointmentId),
                currentUser.UserId.Value,
                request.ExpectedRevision,
    ...
    private static async Task<IResult> ValidateResultAsync(
        Guid appointmentId,
        AssessmentResultTransitionRequest request,
        IMediator mediator,
        ICurrentTenant tenant,
        ICurrentUser currentUser,
        HttpContext context,
    ...
    )
    {
        ...
        if (!currentUser.IsAuthenticated || currentUser.UserId is null)
            return LeadErrors.CurrentUserRequired.ToHttpResult(context);
        Result result = await mediator.Send(
            new ValidateAssessmentResultCommand(
                tenant.OrganizationId.Value,
                new AssessmentAppointmentId(appointmentId),
                currentUser.UserId.Value,
                request.ExpectedRevision,
    ...
    private static async Task<IResult> ShareResultAsync(
        Guid appointmentId,
        AssessmentResultTransitionRequest request,
        IMediator mediator,
        ICurrentTenant tenant,
    ...
    )
    {
        ...
        if (!currentUser.IsAuthenticated || currentUser.UserId is null)
            return LeadErrors.CurrentUserRequired.ToHttpResult(context);
        Result result = await mediator.Send(
            new ShareAssessmentResultCommand(
                tenant.OrganizationId.Value,
                new AssessmentAppointmentId(appointmentId),
                currentUser.UserId.Value,
                request.ExpectedRevision,
...
public sealed record StartAssessmentSessionRequest(
    string QuestionnaireCode,
    int QuestionnaireVersion,
    System.Text.Json.JsonElement QuestionnaireSnapshot
);
