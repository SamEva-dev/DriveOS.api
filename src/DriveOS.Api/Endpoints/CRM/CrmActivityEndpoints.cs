...
{
    public static IEndpointRouteBuilder MapCrmActivityEndpoints(
    ...
    )
    {
        ...
        group
            .MapPost("/activities", CreateUnattachedAsync)
            .RequireAuthorization("Crm.Activities.CreateUnattached");
        group
            .MapPost("/activities/import", ImportAsync)
            .RequireAuthorization("Crm.Activities.Import");
        group
            .MapPost("/activities/{activityId:guid}/attach", AttachAsync)
            .RequireAuthorization("Crm.Activities.Attach");
        group
            .MapPost("/activities/{activityId:guid}/invalidate", InvalidateAsync)
            .RequireAuthorization("Crm.Activities.Invalidate");
        group
            .MapPost("/activities/{activityId:guid}/sync/retry", RetrySyncAsync)
            .RequireAuthorization("Crm.Activities.Sync.Manage");
        group
            .MapPost("/activities/{activityId:guid}/sync/abandon", AbandonSyncAsync)
            .RequireAuthorization("Crm.Activities.Sync.Manage");
        group
            .MapPost("/activities/{activityId:guid}/attachment", UploadAttachmentAsync)
            .DisableAntiforgery()
            .RequireAuthorization("Crm.Activities.Attachments.Upload");
        group
            .MapGet("/activities/{activityId:guid}/attachment", DownloadAttachmentAsync)
            .RequireAuthorization("Crm.Activities.Attachments.Read");
        group
            .MapDelete("/activities/{activityId:guid}/attachment", DeleteAttachmentAsync)
            .RequireAuthorization("Crm.Activities.Attachments.Delete");
        group
            .MapGet("/leads/{leadId:guid}/activities", GetByLeadAsync)
            .RequireAuthorization("Crm.Activities.Read");
        group
            .MapPost("/leads/{leadId:guid}/activities", CreateAsync)
    ...
    private static async Task<IResult> GetPageAsync(
        ...
        bool? syncErrorsOnly,
        bool? duplicatesOnly,
        bool? regularizationOnly,
        bool? unfollowedOnly,
        DateTimeOffset? fromUtc,
        DateTimeOffset? toUtc,
        IActivityReadService service,
    ...
    )
    {
        ...
        var query = new ActivityListQuery(
            ...
            search,
            type,
            advisorUserId.HasValue ? new UserId(advisorUserId.Value) : null,
            leadId,
    ...
    private static async Task<IResult> CreateAsync(
        Guid leadId,
        CreateCrmActivityRequest request,
        IMediator mediator,
        ICurrentTenant tenant,
        ICurrentUser user,
        HttpContext context,
    ...
    )
    {
        ...
        Result<Guid> result = await mediator.Send(
            new CreateCrmActivityCommand(
                ...
                request.ToMetadata(),
                request.NextActionTitle,
                request.NextActionDueAtUtc,
        ...
        return result.IsFailure
            ...
            : Results.Created(
                $"/api/crm/leads/{leadId}/activities/{result.Value}",
                new { activityId = result.Value }
            );
    }

    private static Task<IResult> CreateUnattachedAsync(
        CreateCrmActivityRequest request,
        IMediator mediator,
        ICurrentTenant tenant,
        ICurrentUser user,
    ...
    private static Task<IResult> CreateCoreAsync(
        Guid? leadId,
        CreateCrmActivityRequest request,
        IMediator mediator,
        ICurrentTenant tenant,
    ...
    private static async Task<IResult> CreateWithOptionalLeadAsync(
        Guid? leadId,
        CreateCrmActivityRequest request,
        IMediator mediator,
        ICurrentTenant tenant,
    ...
    )
    {
        ...
        Result<Guid> result = await mediator.Send(
            new CreateCrmActivityCommand(
                tenant.OrganizationId.Value,
                leadId.HasValue ? new LeadId(leadId.Value) : null,
                request.Type,
                request.Direction,
                request.Subject,
                request.Details,
        ...
        return result.IsFailure
            ? result.Error.ToHttpResult(context)
            : Results.Created(
                $"/api/crm/activities/{result.Value}",
                new { activityId = result.Value }
            );
    }
    ...
    private static async Task<IResult> AttachAsync(
        Guid activityId,
        AttachActivityRequest request,
        IActivityManagementService service,
        ICurrentTenant tenant,
        HttpContext context,
        CancellationToken ct
    ) =>
        await Manage(
            ...
            tenant,
            context,
            (org, id) => service.AttachAsync(org, new CrmActivityId(id), request.LeadId, ct)
        );

    private static async Task<IResult> ImportAsync(
        ImportCrmActivityRequest request,
        IMediator mediator,
        ICurrentTenant tenant,
        HttpContext context,
        CancellationToken ct
    )
    {
        if (!tenant.HasTenant || tenant.OrganizationId is null)
            return LeadErrors.CurrentTenantRequired.ToHttpResult(context);
        Result<ImportCrmActivityResult> result = await mediator.Send(
            new ImportCrmActivityCommand(
                tenant.OrganizationId.Value,
                request.LeadId.HasValue ? new LeadId(request.LeadId.Value) : null,
                request.Type,
                request.Direction,
                request.Subject,
                request.Details,
    ...
    )
    {
        ...
        return await Manage(
            ...
            context,
            (org, id) =>
                service.InvalidateAsync(
                    org,
                    new CrmActivityId(id),
                    user.UserId.Value,
                    request.Reason,
                    ct
                )
    ...
    private static async Task<IResult> RetrySyncAsync(
        Guid activityId,
        IActivityManagementService service,
        ICurrentTenant tenant,
        HttpContext context,
        ...
        await Manage(
            ...
            tenant,
            context,
            (org, id) => service.RetrySyncAsync(org, new CrmActivityId(id), ct)
        );

    private static async Task<IResult> AbandonSyncAsync(
        Guid activityId,
        IActivityManagementService service,
        ICurrentTenant tenant,
        HttpContext context,
        ...
        await Manage(
            ...
            tenant,
            context,
            (org, id) => service.AbandonSyncAsync(org, new CrmActivityId(id), ct)
        );

    private static async Task<IResult> Manage(
        Guid id,
        IActivityManagementService _,
        ICurrentTenant tenant,
        HttpContext context,
        Func<OrganizationId, Guid, Task<Result>> action
    )
    {
        if (!tenant.HasTenant || tenant.OrganizationId is null)
            return LeadErrors.CurrentTenantRequired.ToHttpResult(context);
    ...
    private static async Task<IResult> UploadAttachmentAsync(
        Guid activityId,
        IFormFile file,
        IActivityAttachmentService service,
        ICurrentTenant tenant,
        HttpContext context,
        CancellationToken ct
    )
    {
        ...
        Result result = await service.UploadAsync(
            tenant.OrganizationId.Value,
            new CrmActivityId(activityId),
            file.FileName,
            file.ContentType,
            file.Length,
            stream,
...
public sealed record CreateCrmActivityRequest(
    CrmActivityType Type,
    ...
    bool RequiresRegularization = false,
    string? AttachmentName = null,
    string? AttachmentReference = null,
    string? NextActionTitle = null,
    DateTimeOffset? NextActionDueAtUtc = null,
    DriveOS.Modules.CRM.Domain.Tasks.CrmTaskType NextActionType =
        DriveOS.Modules.CRM.Domain.Tasks.CrmTaskType.FollowUp
)
{
    public CrmActivityMetadata ToMetadata() =>
        CrmActivityMetadata.Manual(
            Result,
            DurationMinutes,
            IsInternal,
            IsUnfollowed,
...
public sealed record ImportCrmActivityRequest(
    Guid? LeadId,
    CrmActivityType Type,
    CrmActivityDirection Direction,
    string Subject,
