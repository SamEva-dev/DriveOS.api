...
{
    public static IEndpointRouteBuilder MapCrmDashboardEndpoints(
        this IEndpointRouteBuilder endpoints
    )
    {
        endpoints
            .MapGet("/api/crm/dashboard", GetAsync)
    ...
    )
    {
        ...
        if (normalizedScope is not ("branch" or "organization" or "network"))
            return Results.BadRequest(
                new
                {
                    key = "Crm.Dashboard.Scope.Unsupported",
        ...
        if (normalizedScope == "branch" && (!branchId.HasValue || branchId == Guid.Empty))
            return Results.BadRequest(
                new
                {
                    key = "Crm.Dashboard.BranchId.Required",
        ...
        if (fromUtc.HasValue && toUtc.HasValue && fromUtc.Value >= toUtc.Value)
            return Results.BadRequest(
                new
                {
                    key = "Crm.Dashboard.Period.Invalid",
        ...
        if (assignedAdvisorId == Guid.Empty)
            return Results.BadRequest(
                new
                {
                    key = "Crm.Dashboard.Advisor.Invalid",
        ...
        if (
            ...
            return Results.BadRequest(
                new
                {
                    key = "Crm.Dashboard.Source.Unsupported",
        ...
        if (!string.IsNullOrWhiteSpace(status) && !Enum.TryParse<LeadStatus>(status, true, out _))
            return Results.BadRequest(
                new
                {
                    key = "Crm.Dashboard.Status.Unsupported",
        ...
        var branchRows = await organizationsDbContext
        ...
        if (normalizedScope == "branch" && !availableBranches.Any(x => x.Id == branchId!.Value))
            return Results.BadRequest(
                new
                {
                    key = "Crm.Dashboard.Branch.NotAvailable",
        ...
        {
            var currentOrganization = await organizationsDbContext
            ...
            if (currentOrganization is null)
                return Results.BadRequest(
                    new
                    {
                        key = "Crm.Dashboard.Network.CurrentOrganizationNotFound",
                        messageKey = "errors.crm.dashboard.networkCurrentOrganizationNotFound",
                    }
                );

            OrganizationId? networkOrganizationId =
                currentOrganization.Type == OrganizationType.DrivingSchoolNetwork
                    ? currentOrganization.Id
                    : await organizationsDbContext
                        .NetworkOrganizationMemberships.AsNoTracking()
                        .Where(x =>
                            x.MemberOrganizationId == currentOrganizationId && x.EndedAtUtc == null
                        )
            ...
            if (!networkOrganizationId.HasValue)
                return Results.BadRequest(
                    new
                    {
                        key = "Crm.Dashboard.Network.NotAvailable",
                        messageKey = "errors.crm.dashboard.networkNotAvailable",
                    }
                );

            OrganizationId[] memberIds = await organizationsDbContext
                .NetworkOrganizationMemberships.AsNoTracking()
                .Where(x =>
                    x.NetworkOrganizationId == networkOrganizationId.Value && x.EndedAtUtc == null
                )
                .Select(x => x.MemberOrganizationId)
                .ToArrayAsync(ct);
            scopedOrganizationIds =
                memberIds.Length == 0 ? [networkOrganizationId.Value] : memberIds;

            includedOrganizations = await organizationsDbContext
                .Organizations.AsNoTracking()
                .Where(x =>
                    scopedOrganizationIds.Contains(x.Id) && x.Status != OrganizationStatus.Closed
                )
                .OrderBy(x => x.LegalName)
                .Select(x => new CrmDashboardOrganizationScope(
                    x.Id.Value,
                    x.LegalName,
                    x.Type == OrganizationType.DrivingSchoolNetwork
                ))
                .ToArrayAsync(ct);
            scopedOrganizationIds = includedOrganizations
                .Select(x => new OrganizationId(x.Id))
                .ToArray();
        }
        ...
        Result<CrmDashboardResponse> result = await mediator.Send(
            new GetCrmDashboardQuery(scopedOrganizationIds, normalizedScope, branchId, filters),
            ct
        );
