...
{
    public static IEndpointRouteBuilder MapOrganizationSequenceEndpoints(
    ...
    )
    {
        ...
        group
            .MapGet("/{sequenceId:guid}", GetByIdAsync)
    ...
    {
        if (!TryOrganization(organizationId, tenant, context, out var orgId, out var failure))
            return failure!;
        BranchId? branch = TryBranch(branchId, context, out failure);
        if (failure is not null)
            return failure;
        Result<IReadOnlyList<OrganizationSequenceListItem>> result = await mediator.Send(
            new GetOrganizationSequencesQuery(orgId, branch),
            ct
        );
        if (result.IsFailure)
        ...
        return Results.Ok(
            result
                .Value.Select(x =>
                    mapper.Map<OrganizationSequenceListItem, OrganizationSequenceListItemContract>(
                        x
                    )
                )
                .ToArray()
    ...
    )
    {
        if (
            ...
            return failure!;
        Result<OrganizationSequenceResponse> result = await mediator.Send(
            new GetOrganizationSequenceByIdQuery(orgId, id),
            ct
        );
        return result.IsSuccess
            ? Results.Ok(
                mapper.Map<OrganizationSequenceResponse, OrganizationSequenceResponseContract>(
                    result.Value
                )
            )
    ...
    private static async Task<IResult> CreateAsync(
        Guid organizationId,
        CreateOrganizationSequenceRequest request,
        IMediator mediator,
        IObjectMapper mapper,
        ICurrentTenant tenant,
        HttpContext context,
    ...
    )
    {
        ...
        if (failure is not null)
            return failure;
        var model = new CreateOrganizationSequenceApiModel(
            orgId,
            branch,
            request.Scope,
            request.Code,
        ...
        Result<OrganizationSequenceId> result = await mediator.Send(
            mapper.Map<CreateOrganizationSequenceApiModel, CreateOrganizationSequenceCommand>(
                model
            ),
            ct
        );
        return result.IsSuccess
            ? Results.Created(
                $"/api/organizations/{organizationId}/sequences/{result.Value.Value}",
    ...
    private static async Task<IResult> ReserveAsync(
        Guid organizationId,
        ReserveOrganizationSequenceNumberRequest request,
        IMediator mediator,
        IObjectMapper mapper,
        ICurrentTenant tenant,
        HttpContext context,
    ...
    )
    {
        ...
        return result.IsSuccess
            ? Results.Ok(new OrganizationSequenceNumberResponse(result.Value))
            : result.Error.ToHttpResult(context);
    }
    ...
    private static Task<IResult> SuspendAsync(
        Guid organizationId,
        Guid sequenceId,
        ChangeOrganizationSequenceStatusRequest request,
        IMediator mediator,
        IObjectMapper mapper,
        ICurrentTenant tenant,
        HttpContext context,
    ...
    )
    {
        if (
            ...
            return failure!;
        var model = new ChangeOrganizationSequenceStatusApiModel(
            orgId,
            id,
        ...
        Result result = action switch
        {
            ...
            "reactivate" => await mediator.Send(
                mapper.Map<
                    ChangeOrganizationSequenceStatusApiModel,
    ...
    private static bool TryIds(
        ...
        out OrganizationSequenceId sequenceId,
        out IResult? failure
    )
    {
        sequenceId = default;
