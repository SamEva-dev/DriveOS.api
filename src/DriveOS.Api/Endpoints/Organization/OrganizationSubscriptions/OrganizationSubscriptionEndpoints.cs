...
public static class OrganizationSubscriptionEndpoints
{
    ...
    )
    {
        ...
        group
            .MapPost("/change-plan", ChangePlanAsync)
        ...
        group
            .MapPost(
                ...
                (
                    Guid organizationId,
                    ChangeOrganizationSubscriptionStatusRequest request,
                    IMediator mediator,
        ...
        group
            .MapGet("/limits/{limitCode}", CheckLimitAsync)
    ...
    )
    {
        if (
            ...
            return failure!;
        Result<OrganizationSubscriptionResponse> result = await mediator.Send(
            new GetOrganizationSubscriptionQuery(id),
            ct
        );
        if (result.IsFailure)
            return result.Error.ToHttpResult(context);
        return Results.Ok(
            mapper.Map<OrganizationSubscriptionResponse, OrganizationSubscriptionResponseContract>(
                result.Value
            )
        );
    }
    ...
    private static async Task<IResult> CreateAsync(
        Guid organizationId,
        CreateOrganizationSubscriptionRequest request,
        IMediator mediator,
        IObjectMapper mapper,
        ICurrentTenant tenant,
        HttpContext context,
    ...
    )
    {
        if (
            ...
            return failure!;
        var model = new CreateOrganizationSubscriptionApiModel(
            id,
            request.PlanCode,
            request.Status,
            ...
            request.TrialStartsAtUtc,
            request.TrialEndsAtUtc,
            request.ExternalProvider,
            request.ExternalSubscriptionId
        );
        CreateOrganizationSubscriptionCommand command = mapper.Map<
    ...
    private static async Task<IResult> ChangePlanAsync(
        Guid organizationId,
        ChangeOrganizationSubscriptionPlanRequest request,
        IMediator mediator,
        IObjectMapper mapper,
        ICurrentTenant tenant,
        ICurrentUser user,
    ...
    )
    {
        if (
            ...
            return failure!;
        if (!TryGetActor(user, context, out UserId actor, out failure))
            return failure!;
        var model = new ChangeOrganizationSubscriptionPlanApiModel(
            id,
            request.PlanCode,
            request.EntitlementCodes,
            request.Limits,
    ...
    )
    {
        ...
        if (!TryGetActor(user, context, out UserId actor, out failure))
            return failure!;
        var model = new ChangeOrganizationSubscriptionStatusApiModel(
            id,
            targetStatus,
            request.PeriodStartsAtUtc,
            request.PeriodEndsAtUtc,
            request.ExpectedVersion,
            request.Reason,
            actor
        );
    ...
    private static async Task<IResult> CancelAsync(
        Guid organizationId,
        CancelOrganizationSubscriptionRequest request,
        IMediator mediator,
        IObjectMapper mapper,
        ICurrentTenant tenant,
        ICurrentUser user,
    ...
    )
    {
        ...
        if (!TryGetActor(user, context, out UserId actor, out failure))
            return failure!;
        var model = new CancelOrganizationSubscriptionApiModel(
            id,
    ...
    private static async Task<IResult> CheckEntitlementAsync(
        Guid organizationId,
    ...
    )
    {
        ...
        Result<bool> result = await mediator.Send(
            new CheckOrganizationEntitlementQuery(id, entitlementCode),
            ct
        );
        return result.IsSuccess
            ? Results.Ok(new OrganizationEntitlementCheckResponse(entitlementCode, result.Value))
            : result.Error.ToHttpResult(context);
    }

    private static async Task<IResult> CheckLimitAsync(
        Guid organizationId,
        string limitCode,
        long currentUsage,
    ...
    )
    {
        if (
            ...
            return failure!;
        Result<OrganizationLimitCheckResponse> result = await mediator.Send(
            new CheckOrganizationLimitQuery(id, limitCode, currentUsage, requestedIncrease),
            ct
        );
        if (result.IsFailure)
            return result.Error.ToHttpResult(context);
        return Results.Ok(
            new OrganizationLimitCheckResponseContract(
                limitCode,
                (int)result.Value.Availability,
                result.Value.Limit,
                result.Value.CurrentUsage,
                result.Value.RequestedIncrease
            )
        );
    }
    ...
    )
    {
        ...
        if (actor.IsEmpty)
        {
            failure = Error
                .Forbidden(
                    "errors.organizationSubscription.actor.required",
