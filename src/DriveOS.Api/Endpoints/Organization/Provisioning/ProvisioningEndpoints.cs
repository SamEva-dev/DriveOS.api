...
public static class ProvisioningEndpoints
{
        ...
        this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapGet("/{organizationId:guid}", VerifyOrganizationAsync)
            .WithName("VerifyDriveOsOrganization")
            .WithSummary("Vérifier une organisation DriveOS depuis AuthGate")
            .Produces<VerifyProvisionedOrganizationResponse>(
        ...
        CancellationToken cancellationToken)
    {
        ...
        Result<OrganizationId> result = await mediator.Send(
            command,
            cancellationToken);
        ...
        return Results.Created(
            ...
            new ProvisionOrganizationResponse(
                organizationId,
                OrganizationStatus.Draft.ToString()));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        Result<OrganizationResponse> result = await mediator.Send(
            query,
            cancellationToken);
        ...
        bool canAuthenticate = organization.Status is
            nameof(OrganizationStatus.Draft) or
            nameof(OrganizationStatus.PendingActivation) or
            nameof(OrganizationStatus.Active) or
            nameof(OrganizationStatus.Restricted);
