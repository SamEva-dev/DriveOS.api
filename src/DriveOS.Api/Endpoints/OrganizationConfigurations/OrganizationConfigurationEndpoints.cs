...
namespace DriveOS.Api.Endpoints.OrganizationConfigurations;
...
public static class OrganizationConfigurationEndpoints
{
        ...
        this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapGet("/{configurationId:guid}", GetByIdAsync)
        ...
        group.MapPut("/{configurationId:guid}", UpdateDraftAsync)
    ...
    {
        if (!TryGetScopedOrganizationId(organizationId, currentTenant, httpContext,
                out OrganizationId id, out IResult? failure))
            return failure!;

        Result<IReadOnlyList<OrganizationConfigurationListItemResponse>> result =
            await mediator.Send(new GetOrganizationConfigurationVersionsQuery(id), cancellationToken);

        if (result.IsFailure) return result.Error.ToHttpResult(httpContext);

        var response = result.Value
            .Select(item => mapper.Map<OrganizationConfigurationListItemResponse,
                OrganizationConfigurationListItemResponseContract>(item))
            .ToArray();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!TryGetConfigurationId(configurationId, httpContext,
                out OrganizationConfigurationId configId, out failure))
            return failure!;

        Result<OrganizationConfigurationResponse> result = await mediator.Send(
            new GetOrganizationConfigurationQuery(id, configId), cancellationToken);

        if (result.IsFailure) return result.Error.ToHttpResult(httpContext);

        return Results.Ok(mapper.Map<OrganizationConfigurationResponse,
            OrganizationConfigurationResponseContract>(result.Value));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var model = new CreateOrganizationConfigurationDraftApiModel(
            id, request.VersionNumber, request.CountryCode, request.PayloadJson);

        CreateOrganizationConfigurationDraftCommand command = mapper.Map<
            CreateOrganizationConfigurationDraftApiModel,
            CreateOrganizationConfigurationDraftCommand>(model);

        Result<OrganizationConfigurationId> result =
    ...
    private static async Task<IResult> UpdateDraftAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (!TryGetIds(organizationId, configurationId, currentTenant, httpContext,
                out OrganizationId id, out OrganizationConfigurationId configId,
                out IResult? failure))
            return failure!;

        var model = new UpdateOrganizationConfigurationDraftApiModel(
            id, configId, request.PayloadJson, request.ExpectedRevision);

        Result result = await mediator.Send(mapper.Map<
            UpdateOrganizationConfigurationDraftApiModel,
            UpdateOrganizationConfigurationDraftCommand>(model), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var model = new PublishOrganizationConfigurationApiModel(
            id, configId, request.EffectiveFromUtc,
            request.EffectiveToUtc, request.ExpectedRevision);

        Result result = await mediator.Send(mapper.Map<
            PublishOrganizationConfigurationApiModel,
            PublishOrganizationConfigurationCommand>(model), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var model = new ArchiveOrganizationConfigurationApiModel(
            id, configId, request.ExpectedRevision);

        Result result = await mediator.Send(mapper.Map<
            ArchiveOrganizationConfigurationApiModel,
            ArchiveOrganizationConfigurationCommand>(model), cancellationToken);
        ...
        out IResult? failure)
    {
        ...
        if (!TryGetScopedOrganizationId(rawOrganizationId, currentTenant, httpContext,
                out organizationId, out failure))
            return false;

        return TryGetConfigurationId(rawConfigurationId, httpContext,
            out configurationId, out failure);
    }
        ...
        out IResult? failure)
    {
        ...
        {
            failure = OrganizationConfigurationErrors.EmptyOrganizationId
        ...
        if (!currentTenant.HasTenant ||
            currentTenant.OrganizationId != organizationId)
        {
        ...
        out IResult? failure)
    {
        ...
        {
            failure = OrganizationConfigurationErrors.EmptyId
                .ToHttpResult(httpContext);
            return false;
        }
