...
internal sealed class TrainingContractSourceGateway(CrmDbContext crm, StudentsDbContext students, OrganizationsDbContext organizations) : ITrainingContractSourceGateway
{
    ...
    {
        var enrollment = await students.Enrollments.AsNoTracking().SingleOrDefaultAsync(x => x.OrganizationId == organizationId && x.Id == enrollmentId, ct);
        ...
        if (enrollment.SourceLeadId is null || enrollment.SourceLeadId != offer.LeadId || !string.Equals(enrollment.TrainingCode, offer.TrainingCode, StringComparison.OrdinalIgnoreCase) || (offer.BranchId.HasValue && offer.BranchId.Value != enrollment.BranchId))
            return Result.Failure<TrainingContractSourceSnapshot>(CreateTrainingContractErrors.SourceMismatch);
        var student = await students.Students.AsNoTracking().SingleOrDefaultAsync(x => x.OrganizationId == organizationId && x.Id == enrollment.StudentId, ct);
        if (student is null) return Result.Failure<TrainingContractSourceSnapshot>(CreateTrainingContractErrors.StudentNotFound);
        ...
        return Result.Success(new TrainingContractSourceSnapshot(organizationId, enrollment.BranchId, enrollment.StudentId,
            $"{student.FirstName} {student.LastName}".Trim(), organization.LegalName, offer.Id, offer.Version,
