    ...
    : ICommandHandler<CreateCrmActivityCommand, Guid>
{
    public async Task<Result<Guid>> Handle(CreateCrmActivityCommand command, CancellationToken ct)
    {
        ...
        Result<CrmActivity> result = CrmActivity.Create(CrmActivityId.New(), command.OrganizationId,
            ...
            command.Details, command.OccurredAtUtc, command.AdvisorUserId, command.Metadata);

        if (result.IsFailure)
            return Result.Failure<Guid>(result.Error);

        activities.Add(result.Value);
        if (!string.IsNullOrWhiteSpace(command.NextActionTitle) && command.NextActionDueAtUtc.HasValue)
        {
            if (!command.LeadId.HasValue)
                return Result.Failure<Guid>(CrmActivityErrors.NextActionRequiresLead);
            Result<CrmTask> task = CrmTask.Create(CrmTaskId.New(), command.OrganizationId,
                command.LeadId.Value, command.NextActionType, command.NextActionTitle,
                command.Details, command.NextActionDueAtUtc.Value, command.AdvisorUserId);
            if (task.IsFailure) return Result.Failure<Guid>(task.Error);
