...
public sealed class ImportCrmActivityCommandValidator : AbstractValidator<ImportCrmActivityCommand>
{
    public ImportCrmActivityCommandValidator()
    {
        ...
        RuleFor(x => x.SyncErrorKey).NotEmpty().When(x => x.SyncStatus == CrmActivitySyncStatus.Failed);
        RuleFor(x => x.SyncStatus).Must(x => x is CrmActivitySyncStatus.Pending
            or CrmActivitySyncStatus.Synchronized or CrmActivitySyncStatus.Failed);
    }
}
