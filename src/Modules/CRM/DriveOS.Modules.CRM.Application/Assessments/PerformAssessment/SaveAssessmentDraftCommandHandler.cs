...
internal sealed class SaveAssessmentDraftCommandHandler(
...
) : ICommandHandler<SaveAssessmentDraftCommand>
{
    ...
    )
    {
        ...
        Result result = session.SaveDraft(
            command.AnswersJson,
            command.FactualObservations,
            command.PedagogicalInterpretation,
            command.Recommendation,
            command.InternalNotes,
            command.ProspectComment,
            command.DraftCompleted,
            command.SavedAtUtc
        );
        if (result.IsFailure)
            return result;
        sessions.AddRevision(
            AssessmentSessionRevision.Capture(session, command.SavedByUserId, command.SavedAtUtc)
        );
