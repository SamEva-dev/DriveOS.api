...
internal sealed class StartAssessmentCommandHandler(
    IAssessmentAppointmentRepository appointments,
    IAssessmentSessionRepository sessions,
    ICrmUnitOfWork unitOfWork
) : ICommandHandler<StartAssessmentCommand, Guid>
{
    public async Task<Result<Guid>> Handle(
        StartAssessmentCommand command,
        CancellationToken cancellationToken
    )
    {
        AssessmentAppointment? appointment = await appointments.GetByIdAsync(
            command.OrganizationId,
            command.AppointmentId,
            cancellationToken
        );
        if (appointment is null)
            return Result.Failure<Guid>(AssessmentSessionErrors.AppointmentNotFound);
        if (
            appointment.Status
            is not (
                AssessmentAppointmentStatus.Scheduled
        ...
        if (
            await sessions.ExistsForAppointmentAsync(
                command.OrganizationId,
                command.AppointmentId,
                cancellationToken
            )
        ...
        Result<AssessmentSession> created = AssessmentSession.Start(
            AssessmentSessionId.New(),
            command.OrganizationId,
            appointment.Id,
            appointment.LeadId,
            command.EvaluatorUserId,
            command.QuestionnaireCode,
            command.QuestionnaireVersion,
            command.QuestionnaireSnapshotJson,
