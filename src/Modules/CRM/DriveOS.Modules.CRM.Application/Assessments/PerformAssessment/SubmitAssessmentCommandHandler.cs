...
internal sealed class SubmitAssessmentCommandHandler(
...
) : ICommandHandler<SubmitAssessmentCommand>
{
    ...
    )
    {
        ...
        AssessmentAppointment? appointment = await appointments.GetByIdForUpdateAsync(
            command.OrganizationId,
            command.AppointmentId,
            cancellationToken
        );
        if (appointment is null)
            return Result.Failure(AssessmentSessionErrors.AppointmentNotFound);
        ...
        sessions.AddRevision(
            AssessmentSessionRevision.Capture(
                session,
