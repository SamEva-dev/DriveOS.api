...
public sealed record CrmDashboardPriority(Guid LeadId, string FirstName,
    string LastName, string Kind, string Label, DateTimeOffset? DueAtUtc);
public sealed record CrmDashboardPipelineStage(string Status, int Count);
public sealed record CrmDashboardActivity(Guid Id, Guid LeadId, string FirstName,
    string LastName, string Type, string Direction, string Subject, DateTimeOffset OccurredAtUtc);
public sealed record CrmDashboardTask(Guid Id, Guid LeadId, string FirstName,
    string LastName, string Type, string Title, DateTimeOffset DueAtUtc, bool IsOverdue);
public sealed record CrmDashboardAppointment(Guid Id, Guid LeadId, string FirstName,
    string LastName, string Type, string DeliveryMode, string Status,
    DateTimeOffset StartsAtUtc, DateTimeOffset EndsAtUtc, string? LocationDetails);
...
public sealed record CrmDashboardInactiveLead(Guid LeadId, string FirstName,
    string LastName, string Status, DateTimeOffset LastInteractionAtUtc, int InactiveDays);
