...
public enum LeadBulkActionType
{
    AssignAdvisor = 0,
    ChangeStatus = 1,
}

public sealed record LeadBulkActionInput(
    IReadOnlyCollection<Guid> LeadIds,
    LeadBulkActionType Action,
    Guid? AdvisorId,
...
public sealed record LeadBulkActionResult(
    int Requested,
    int Succeeded,
    int Failed,
...
{
    Task<LeadBulkActionResult> ExecuteAsync(
