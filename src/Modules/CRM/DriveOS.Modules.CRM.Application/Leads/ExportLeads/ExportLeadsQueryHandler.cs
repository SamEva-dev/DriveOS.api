    ...
    : IQueryHandler<ExportLeadsQuery, LeadExportFile>
{
    ...
    public async Task<Result<LeadExportFile>> Handle(
    ...
    )
    {
        ...
        var csv = new StringBuilder(
            "Id,FirstName,LastName,Email,Phone,LicenseCategory,Transmission,Source,BranchId,AdvisorId,Status,CreatedAtUtc,LastModifiedAtUtc\r\n"
        );
        foreach (LeadExportRow row in rows)
        {
            ...
            [
                ...
                row.BranchId?.ToString("D") ?? string.Empty,
                row.AssignedAdvisorId?.ToString("D") ?? string.Empty,
                row.Status.ToString(),
        ...
        return Result.Success(
            new LeadExportFile(
                content,
                $"driveos-prospects-{DateTime.UtcNow:yyyyMMdd-HHmmss}.csv",
                rows.Count
            )
        );
    }
