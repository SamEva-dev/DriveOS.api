...
internal sealed class SubmitCommercialOfferForReviewCommandHandler(
    ICommercialOfferRepository offers, ICrmUnitOfWork unitOfWork)
    : ICommandHandler<SubmitCommercialOfferForReviewCommand>
{
    public async Task<Result> Handle(SubmitCommercialOfferForReviewCommand command, CancellationToken cancellationToken)
    {
...
{
    public async Task<Result> Handle(ApproveCommercialOfferCommand command, CancellationToken cancellationToken)
    {
        CommercialOffer? offer = await offers.GetForUpdateAsync(command.OrganizationId, command.OfferId, cancellationToken);
        if (offer is null) return Result.Failure(CommercialOfferErrors.NotFound);
