...
internal sealed class CreateCommercialOfferVariantCommandHandler(
    ...
    IClock clock
) : ICommandHandler<CreateCommercialOfferVariantCommand, Guid>
{
    public async Task<Result<Guid>> Handle(
        CreateCommercialOfferVariantCommand command,
        CancellationToken cancellationToken
    )
    {
        CommercialOffer? source = await offers.GetByIdAsync(
            command.OrganizationId,
            command.SourceOfferId,
            cancellationToken
        );
        if (source is null)
            return Result.Failure<Guid>(CommercialOfferErrors.NotFound);
        if (
            source.Status
            is not (
                CommercialOfferStatus.Draft
                or CommercialOfferStatus.InternalReview
                or CommercialOfferStatus.Approved
                or CommercialOfferStatus.Sent
        ...
        string[] mandatoryDescriptions = source
            .Lines.Where(x => x.Mandatory)
            .Select(x => x.Description.Trim())
            .ToArray();
        bool removesMandatoryLine = mandatoryDescriptions.Any(required =>
            !command.Lines.Any(x =>
                x.Mandatory
                && string.Equals(x.Description.Trim(), required, StringComparison.OrdinalIgnoreCase)
            )
        );
        if (removesMandatoryLine)
            return Result.Failure<Guid>(CommercialOfferErrors.MandatoryLineRequired);
        ...
        if (variant.IsFailure)
            return Result.Failure<Guid>(variant.Error);
