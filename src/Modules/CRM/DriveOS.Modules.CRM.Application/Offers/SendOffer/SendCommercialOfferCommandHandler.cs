...
internal sealed class SendCommercialOfferCommandHandler(
    ...
    IClock clock
) : ICommandHandler<SendCommercialOfferCommand, SendCommercialOfferResponse>
{
    public async Task<Result<SendCommercialOfferResponse>> Handle(
        SendCommercialOfferCommand command,
    ...
    )
    {
        ...
        if (command.Recipients.Count == 0)
            return Result.Failure<SendCommercialOfferResponse>(
                CommercialOfferErrors.RecipientRequired
            );
        if (
            command.Recipients.Any(x =>
                string.IsNullOrWhiteSpace(x.DisplayName)
                || string.IsNullOrWhiteSpace(x.Address)
                || x.DisplayName.Length > 200
                || x.Address.Length > 320
            )
        )
            return Result.Failure<SendCommercialOfferResponse>(
                CommercialOfferErrors.InvalidRecipient
            );
        ...
        if (command.Channel.RequiresSecureLink())
        {
            if (command.SecureLinkLifetimeHours is < 1 or > 720)
                return Result.Failure<SendCommercialOfferResponse>(
                    CommercialOfferErrors.SecureLinkExpired
                );
            rawToken = Convert.ToHexString(RandomNumberGenerator.GetBytes(32)).ToLowerInvariant();
            tokenHash = Convert
                .ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(rawToken)))
                .ToLowerInvariant();
        ...
        Result prepared = offer.PrepareDelivery(
            command.Channel,
            recipientsJson,
            command.Subject,
            command.Message,
        ...
        if (command.Channel == OfferDeliveryChannel.Printed)
        {
            ...
            if (sent.IsFailure)
                return Result.Failure<SendCommercialOfferResponse>(sent.Error);
        }

        await unitOfWork.CommitAsync(cancellationToken);
        return Result.Success(
            new SendCommercialOfferResponse(
                offer.Id.Value,
                offer.Status.ToString(),
                offer.DeliveryStatus!.Value.ToString(),
                rawToken,
                expiresAtUtc
            )
        );
