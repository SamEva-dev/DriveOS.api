...
public sealed class CloseCrmTaskCommandHandler(ICrmTaskRepository repository, ICrmUnitOfWork unitOfWork)
    : ICommandHandler<CloseCrmTaskCommand>
{
    public async Task<Result> Handle(CloseCrmTaskCommand command, CancellationToken ct)
    {
        ...
        await unitOfWork.CommitAsync(ct); return Result.Success();
    }
}
