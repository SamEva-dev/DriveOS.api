...
public sealed class AssessmentSession : AggregateRoot<AssessmentSessionId>, IAuditableEntity
{
    ...
    private AssessmentSession(AssessmentSessionId id, OrganizationId organizationId, AssessmentAppointmentId appointmentId, LeadId leadId,
        UserId evaluatorUserId, string questionnaireCode, int questionnaireVersion,
        string questionnaireSnapshotJson, DateTimeOffset startedAtUtc) : base(id)
    {
    ...
    public static Result<AssessmentSession> Start(AssessmentSessionId id, OrganizationId organizationId, AssessmentAppointmentId appointmentId,
        LeadId leadId, UserId evaluatorUserId, string questionnaireCode, int questionnaireVersion,
        string questionnaireSnapshotJson, DateTimeOffset startedAtUtc)
    {
        if (id == AssessmentSessionId.Empty || appointmentId == AssessmentAppointmentId.Empty || evaluatorUserId == UserId.Empty)
            return Result.Failure<AssessmentSession>(AssessmentSessionErrors.InvalidIdentifier);
        if (string.IsNullOrWhiteSpace(questionnaireCode) || questionnaireVersion <= 0 || !IsJson(questionnaireSnapshotJson))
            return Result.Failure<AssessmentSession>(AssessmentSessionErrors.InvalidQuestionnaire);

        var session = new AssessmentSession(id, organizationId, appointmentId, leadId, evaluatorUserId,
            questionnaireCode.Trim(), questionnaireVersion, questionnaireSnapshotJson, startedAtUtc);
        session.RaiseDomainEvent(new InitialAssessmentStartedDomainEvent(id, appointmentId, organizationId, evaluatorUserId, session.StartedAtUtc));
        return Result.Success(session);
    }

    public Result SaveDraft(string answersJson, string? factualObservations, string? pedagogicalInterpretation,
        string? recommendation, string? internalNotes, string? prospectComment, bool draftCompleted, DateTimeOffset savedAtUtc)
    {
        ...
        if (TooLong(factualObservations, 8000) || TooLong(pedagogicalInterpretation, 8000) ||
            TooLong(recommendation, 8000) || TooLong(internalNotes, 8000) || TooLong(prospectComment, 4000))
        ...
        Status = draftCompleted ? AssessmentSessionStatus.DraftCompleted : AssessmentSessionStatus.InProgress;
        Revision++;
        LastSavedAtUtc = savedAtUtc.ToUniversalTime();
        RaiseDomainEvent(new InitialAssessmentDraftSavedDomainEvent(Id, OrganizationId, Revision, LastSavedAtUtc.Value));
    ...
    public Result Submit(UserId submittedByUserId, DateTimeOffset submittedAtUtc)
    {
        ...
        RaiseDomainEvent(new InitialAssessmentSubmittedDomainEvent(Id, AppointmentId, OrganizationId, submittedByUserId, Revision, SubmittedAtUtc.Value));
        return Result.Success();
    }

    public Result SaveResult(string resultJson, AssessmentResultConfidence confidence,
        string? aiSuggestionJson, UserId savedByUserId, DateTimeOffset savedAtUtc)
    {
        if (Status != AssessmentSessionStatus.Submitted)
            return Result.Failure(AssessmentSessionErrors.ResultRequiresSubmittedAssessment);
        if (!IsJsonObject(resultJson) || (aiSuggestionJson is not null && !IsJsonObject(aiSuggestionJson)))
        ...
        Revision++;
        LastSavedAtUtc = savedAtUtc.ToUniversalTime();
        RaiseDomainEvent(new InitialAssessmentResultDraftSavedDomainEvent(
            Id, OrganizationId, savedByUserId, Revision, LastSavedAtUtc.Value));
        return Result.Success();
    }

    public Result RequestResultCorrection(string reason, UserId requestedByUserId,
        DateTimeOffset requestedAtUtc)
    {
        ...
        RaiseDomainEvent(new InitialAssessmentResultCorrectionRequestedDomainEvent(
            Id, OrganizationId, requestedByUserId, Revision, LastSavedAtUtc.Value));
        return Result.Success();
    }
    ...
    public Result ValidateResult(UserId validatedByUserId, DateTimeOffset validatedAtUtc)
    {
        ...
        Revision++;
        RaiseDomainEvent(new InitialAssessmentResultValidatedDomainEvent(
            Id, AppointmentId, LeadId, OrganizationId, validatedByUserId, Revision,
    ...
    public Result MarkResultShared(UserId sharedByUserId, DateTimeOffset sharedAtUtc)
    {
        ...
        Revision++;
        RaiseDomainEvent(new InitialAssessmentResultSharedDomainEvent(
            Id, LeadId, OrganizationId, sharedByUserId, Revision, ResultSharedAtUtc.Value));
        return Result.Success();
    }
    ...
    private static bool IsJson(string value) { try { using JsonDocument _ = JsonDocument.Parse(value); return true; } catch (JsonException) { return false; } }
    private static bool IsJsonArray(string value) { try { using JsonDocument doc = JsonDocument.Parse(value); return doc.RootElement.ValueKind == JsonValueKind.Array; } catch (JsonException) { return false; } }
    private static bool IsJsonObject(string value) { try { using JsonDocument doc = JsonDocument.Parse(value); return doc.RootElement.ValueKind == JsonValueKind.Object; } catch (JsonException) { return false; } }
