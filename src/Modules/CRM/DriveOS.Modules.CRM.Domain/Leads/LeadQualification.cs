...
public sealed record LeadQualification
{
    ...
    public static Result<LeadQualification> Create(
        string need,
        string licenseCategory,
        string availability,
        DateOnly? targetDate,
    ...
    )
    {
        ...
        if (normalizedNeed.Length is 0 or > 1000)
            return Result.Failure<LeadQualification>(LeadErrors.QualificationNeedInvalid);
        if (normalizedCategory.Length is 0 or > 30)
            return Result.Failure<LeadQualification>(LeadErrors.QualificationCategoryInvalid);
        if (normalizedAvailability.Length is 0 or > 500)
            return Result.Failure<LeadQualification>(LeadErrors.QualificationAvailabilityInvalid);
        if (!Enum.IsDefined(financing))
            return Result.Failure<LeadQualification>(LeadErrors.QualificationFinancingInvalid);
        if (normalizedNotes?.Length > 2000)
            return Result.Failure<LeadQualification>(LeadErrors.QualificationNotesTooLong);

        return Result.Success(
            new LeadQualification(
                normalizedNeed,
                normalizedCategory,
