...
public sealed class CrmTask : AggregateRoot<CrmTaskId>, IAuditableEntity
{
    ...
    private CrmTask(CrmTaskId id, OrganizationId organizationId, LeadId leadId, CrmTaskType type,
        string title, string? notes, DateTimeOffset dueAtUtc, UserId? assignedToUserId)
        : base(id)
    {
        OrganizationId = organizationId; LeadId = leadId; Type = type; Title = title;
        Notes = notes; DueAtUtc = dueAtUtc; AssignedToUserId = assignedToUserId;
    ...
    { if (CreatedAtUtc == default) { CreatedAtUtc = createdAtUtc; CreatedByUserId = createdByUserId; } }
    public void SetModifiedAudit(DateTimeOffset modifiedAtUtc, UserId? modifiedByUserId)
    { LastModifiedAtUtc = modifiedAtUtc; LastModifiedByUserId = modifiedByUserId; }
}
