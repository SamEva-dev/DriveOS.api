    ...
    : IActivityAttachmentService
{
    ...
    private readonly string root =
        configuration["Crm:ActivityAttachments:RootPath"]
    ...
    public async Task<Result<ActivityAttachmentDownload>> DownloadAsync(
        OrganizationId org,
        CrmActivityId id,
        CancellationToken ct
    )
    {
        CrmActivity? activity = await db
        ...
        if (activity is null)
            return Result.Failure<ActivityAttachmentDownload>(NotFound());
        if (string.IsNullOrWhiteSpace(reference))
            return Result.Failure<ActivityAttachmentDownload>(CrmActivityErrors.AttachmentNotFound);
        ...
        if (!File.Exists(path))
            return Result.Failure<ActivityAttachmentDownload>(
                CrmActivityErrors.AttachmentUnavailable
            );
        return Result.Success(
            new ActivityAttachmentDownload(
                new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read, 81920, true),
                activity.Metadata.AttachmentName!,
                ContentType(activity.Metadata.AttachmentName!)
            )
        );
    }
    ...
    private Task<CrmActivity?> FindAsync(
        OrganizationId org,
        CrmActivityId id,
        CancellationToken ct
