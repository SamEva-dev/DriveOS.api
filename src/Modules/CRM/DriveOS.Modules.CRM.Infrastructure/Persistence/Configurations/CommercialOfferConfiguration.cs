...
internal sealed class CommercialOfferConfiguration : IEntityTypeConfiguration<CommercialOffer>
{
    public void Configure(EntityTypeBuilder<CommercialOffer> b)
    {
        ...
        b.HasKey(x => x.Id);
        b.Property(x => x.Id).HasConversion(x => x.Value, x => new CommercialOfferId(x)).HasColumnName("id").ValueGeneratedNever();
        b.Property(x => x.OrganizationId).HasConversion(x => x.Value, x => new OrganizationId(x)).HasColumnName("organization_id");
        b.Property(x => x.LeadId).HasConversion(x => x.Value, x => new LeadId(x)).HasColumnName("lead_id");
        b.Property(x => x.AssessmentSessionId).HasConversion(x => x.Value, x => new AssessmentSessionId(x)).HasColumnName("assessment_session_id");
        b.Property(x => x.AssessmentRevision).HasColumnName("assessment_revision");
        b.Property(x => x.BranchId).HasConversion(x => x.HasValue ? x.Value.Value : (Guid?)null, x => x.HasValue ? new BranchId(x.Value) : null).HasColumnName("branch_id");
        ...
        b.Property(x => x.EstimatedFundingAmount).HasColumnName("estimated_funding_amount").HasPrecision(18, 2);
        b.Property(x => x.ProspectRemainingAmount).HasColumnName("prospect_remaining_amount").HasPrecision(18, 2);
        ...
        b.Property(x => x.DeliveryStatus).HasColumnName("delivery_status").HasConversion<string>().HasMaxLength(24);
        b.Property(x => x.DeliveryChannel).HasColumnName("delivery_channel").HasConversion<string>().HasMaxLength(24);
        b.Property(x => x.RecipientSnapshotJson).HasColumnName("recipient_snapshot_json").HasColumnType("jsonb");
        ...
        b.Property(x => x.AttachmentSnapshotJson).HasColumnName("attachment_snapshot_json").HasColumnType("jsonb");
        b.Property(x => x.SecureLinkTokenHash).HasColumnName("secure_link_token_hash").HasMaxLength(64);
        ...
        b.Property(x => x.CreatedByUserId).HasConversion(x => x.HasValue ? x.Value.Value : (Guid?)null, x => x.HasValue ? new UserId(x.Value) : null).HasColumnName("created_by_user_id");
        b.Property(x => x.LastModifiedAtUtc).HasColumnName("last_modified_at_utc");
        b.Property(x => x.LastModifiedByUserId).HasConversion(x => x.HasValue ? x.Value.Value : (Guid?)null, x => x.HasValue ? new UserId(x.Value) : null).HasColumnName("last_modified_by_user_id");
        b.HasMany(x => x.Lines).WithOne().HasForeignKey(x => x.OfferId).OnDelete(DeleteBehavior.Cascade);
        b.Navigation(x => x.Lines).UsePropertyAccessMode(PropertyAccessMode.Field);
        b.HasMany(x => x.Interactions).WithOne().HasForeignKey(x => x.OfferId).OnDelete(DeleteBehavior.Cascade);
        b.Navigation(x => x.Interactions).UsePropertyAccessMode(PropertyAccessMode.Field);
        b.HasIndex(x => new { x.OrganizationId, x.LeadId, x.Version }).IsUnique();
        b.HasIndex(x => new { x.OrganizationId, x.AssessmentSessionId });
        b.HasIndex(x => x.SecureLinkTokenHash).IsUnique().HasFilter("secure_link_token_hash IS NOT NULL");
...
internal sealed class OfferInteractionConfiguration : IEntityTypeConfiguration<OfferInteraction>
{
    public void Configure(EntityTypeBuilder<OfferInteraction> b)
    {
        ...
        b.HasKey(x => x.Id);
        b.Property(x => x.Id).HasConversion(x => x.Value, x => new OfferInteractionId(x))
            .HasColumnName("id").ValueGeneratedNever();
        b.Property(x => x.OfferId).HasConversion(x => x.Value, x => new CommercialOfferId(x)).HasColumnName("offer_id");
        b.Property(x => x.Type).HasConversion<string>().HasColumnName("interaction_type").HasMaxLength(40);
        b.Property(x => x.OccurredAtUtc).HasColumnName("occurred_at_utc");
        b.Property(x => x.ActorUserId).HasConversion(x => x.HasValue ? x.Value.Value : (Guid?)null,
            x => x.HasValue ? new UserId(x.Value) : null).HasColumnName("actor_user_id");
...
internal sealed class CommercialOfferLineConfiguration : IEntityTypeConfiguration<CommercialOfferLine>
{
    public void Configure(EntityTypeBuilder<CommercialOfferLine> b)
    {
        ...
        b.Property(x => x.Id).HasColumnName("id")
            .HasConversion(x => x.Value, x => new CommercialOfferLineId(x)).ValueGeneratedNever();
        b.Property(x => x.OfferId).HasConversion(x => x.Value, x => new CommercialOfferId(x)).HasColumnName("offer_id");
        ...
        b.Property(x => x.PriceSource).HasConversion<string>().HasColumnName("price_source").HasMaxLength(32);
        b.Property(x => x.ManualOverrideReason).HasColumnName("manual_override_reason").HasMaxLength(1000);
