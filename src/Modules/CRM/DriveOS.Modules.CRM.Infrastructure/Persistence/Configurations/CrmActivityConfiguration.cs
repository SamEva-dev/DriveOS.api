...
internal sealed class CrmActivityConfiguration : IEntityTypeConfiguration<CrmActivity>
{
    public void Configure(EntityTypeBuilder<CrmActivity> b)
    {
        ...
        b.Property(x => x.Id)
            .HasColumnName("id")
            .HasConversion(x => x.Value, x => new CrmActivityId(x))
            .ValueGeneratedNever();
        ...
        b.Property(x => x.AdvisorUserId)
            .HasColumnName("advisor_user_id")
            .HasConversion(
                x => x.HasValue ? x.Value.Value : (Guid?)null,
                x => x.HasValue ? new UserId(x.Value) : null
            );
        b.Property(x => x.InvalidatedAtUtc).HasColumnName("invalidated_at_utc");
        b.Property(x => x.InvalidatedByUserId)
            .HasColumnName("invalidated_by_user_id")
            .HasConversion(
                x => x.HasValue ? x.Value.Value : (Guid?)null,
                x => x.HasValue ? new UserId(x.Value) : null
            );
        ...
        b.OwnsOne(
            ...
            m =>
            {
                ...
                m.Property(x => x.AttachmentName)
                    .HasColumnName("attachment_name")
                    .HasMaxLength(255);
                m.Property(x => x.AttachmentReference)
