...
internal sealed class LeadConfiguration : IEntityTypeConfiguration<Lead>
{
    public void Configure(EntityTypeBuilder<Lead> builder)
    {
        ...
        builder.Property(lead => lead.ConvertedPersonId)
        ...
        builder.Property(lead => lead.DraftEnrollmentId)
            .HasColumnName("draft_enrollment_id")
            .HasConversion(
                id => id.HasValue ? id.Value.Value : (Guid?)null,
                value => value.HasValue ? new DraftEnrollmentId(value.Value) : null);

        builder.Property(lead => lead.ConvertedAtUtc)
            .HasColumnName("converted_at_utc");

        builder.Property(lead => lead.ClosureReason).HasColumnName("closure_reason")
            .HasConversion<string>().HasMaxLength(50);
        builder.Property(lead => lead.ClosureComment).HasColumnName("closure_comment").HasMaxLength(1000);
        ...
        builder.Property(lead => lead.DormancyResponsibleUserId).HasColumnName("dormancy_responsible_user_id")
            .HasConversion(id => id.HasValue ? id.Value.Value : (Guid?)null,
                value => value.HasValue ? new UserId(value.Value) : null);
        builder.Property(lead => lead.DormancyCampaignCode).HasColumnName("dormancy_campaign_code").HasMaxLength(100);
        builder.Property(lead => lead.ReferredPartnerName).HasColumnName("referred_partner_name").HasMaxLength(200);
        builder.Property(lead => lead.SharedDataDescription).HasColumnName("shared_data_description").HasMaxLength(2000);
        builder.Property(lead => lead.ReferralConsentCollectedAtUtc).HasColumnName("referral_consent_collected_at_utc");
        builder.Property(lead => lead.ReopenedAtUtc).HasColumnName("reopened_at_utc");
        builder.Property(lead => lead.AutomaticFollowUpsEnabled)
            .HasColumnName("automatic_follow_ups_enabled").HasDefaultValue(true).IsRequired();
        ...
        builder.OwnsOne(lead => lead.Qualification, qualification =>
        {
            qualification.Property(value => value.Need).HasColumnName("qualification_need").HasMaxLength(1000);
            qualification.Property(value => value.LicenseCategory).HasColumnName("qualification_license_category").HasMaxLength(30);
            qualification.Property(value => value.Availability).HasColumnName("qualification_availability").HasMaxLength(500);
            qualification.Property(value => value.TargetDate).HasColumnName("qualification_target_date");
            qualification.Property(value => value.Financing).HasColumnName("qualification_financing").HasConversion<string>().HasMaxLength(30);
            qualification.Property(value => value.Notes).HasColumnName("qualification_notes").HasMaxLength(2000);
        });
        ...
        builder.HasIndex(lead => new { lead.OrganizationId, lead.ResumeAtUtc })
