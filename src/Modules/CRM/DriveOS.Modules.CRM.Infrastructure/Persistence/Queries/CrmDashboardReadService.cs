...
internal sealed class CrmDashboardReadService(CrmDbContext context) : ICrmDashboardReadService
{
    public async Task<CrmDashboardResponse> GetAsync(
    ...
    )
    {
        ...
        var tasks = await context
            ...
            .ToListAsync(ct);
        IQueryable<AssessmentAppointment> appointmentsQuery = context
        ...
        if (branchId.HasValue)
        {
            ...
            appointmentsQuery = appointmentsQuery.Where(appointment =>
                appointment.BranchId == scopedBranchId
            );
        }
        ...
        IQueryable<CommercialOffer> offersQuery = context
        ...
        if (branchId.HasValue)
        {
            ...
            offersQuery = offersQuery.Where(offer => offer.BranchId == scopedBranchId);
        }
        ...
        var pendingOffers = latestOffers
            .Where(x => x.Status == CommercialOfferStatus.Sent && x.ValidUntilUtc >= nowUtc)
            .ToArray();
        ...
        LeadId[] failedConversionLeadIds = await context
            ...
            .Select(x => x.LeadId)
            .ToArrayAsync(ct);
        LeadId[] assessmentToValidateLeadIds = await context
        ...
        var firstActivities = activities
            .GroupBy(x => x.LeadId)
        ...
        double? firstContactDelay =
            firstActivities.Count == 0
                ? null
                : Math.Round(
                    leadRows
                        .Where(x => firstActivities.ContainsKey(x.Id))
                        .Average(x =>
                            Math.Max(0, (firstActivities[x.Id] - x.CreatedAtUtc).TotalHours)
                        ),
                    1
                );

        var priorities = tasks
            .Where(x => x.DueAtUtc < nowUtc)
            .Take(5)
            .Select(x => new CrmDashboardPriority(
                x.LeadId,
                names[x.LeadId].FirstName,
                names[x.LeadId].LastName,
                "OverdueTask",
                x.Title,
                x.DueAtUtc
            ))
            .ToList();
        foreach (
            var offer in pendingOffers
                .Where(x => x.ValidUntilUtc <= nowUtc.AddDays(7))
                .OrderBy(x => x.ValidUntilUtc)
        )
        {
            if (priorities.Count >= 8 || !names.TryGetValue(offer.LeadId, out var name))
                break;
            priorities.Add(
                new CrmDashboardPriority(
                    offer.LeadId,
                    name.FirstName,
                    name.LastName,
                    "OfferExpiring",
        ...
        {
            if (priorities.Count >= 8 || !names.TryGetValue(leadId.Value, out var name))
                break;
            priorities.Add(
                new CrmDashboardPriority(
                    leadId.Value,
        ...
        foreach (LeadId leadId in failedConversionLeadIds)
        {
            ...
            priorities.Add(
                new CrmDashboardPriority(
                    ...
                    name.LastName,
                    "ConversionFailed",
                    "Student conversion failed",
                    null
                )
            );
        }
        foreach (
            var lead in leadRows
                .Where(x =>
                    x.Status == LeadStatus.Dormant
                    && x.ResumeAtUtc.HasValue
                    && x.ResumeAtUtc.Value <= nowUtc.AddDays(7)
                )
        ...
        var inactive = leadRows
            .Where(x => x.Status is not LeadStatus.Won and not LeadStatus.Lost)
            .Select(x => new
            {
                Lead = x,
                Last = lastActivities.GetValueOrDefault(
                    x.Id,
                    x.LastModifiedAtUtc ?? x.CreatedAtUtc
                ),
            })
            .Where(x => x.Last < inactiveBefore)
            .OrderBy(x => x.Last)
            .Take(10)
            .Select(x => new CrmDashboardInactiveLead(
                x.Lead.Id,
                x.Lead.FirstName,
                x.Lead.LastName,
                x.Lead.Status.ToString(),
                x.Last,
                Math.Max(0, (int)(nowUtc - x.Last).TotalDays)
            ))
            .ToArray();
        ...
        return new CrmDashboardResponse(
            ...
            branchId.HasValue ? "Branch"
                : scopedOrganizationIds.Length > 1 ? "Network"
                : "Organization",
            branchId,
            new CrmDashboardKpis(
                ...
                leadRows.Count(x => x.AssignedAdvisorId == null),
                expiringOffers
            ),
            priorities,
            Enum.GetValues<LeadStatus>()
                .Select(status => new CrmDashboardPipelineStage(
                    status.ToString(),
                    leadRows.Count(x => x.Status == status)
                ))
                .ToArray(),
            activities
                .Take(10)
                .Select(x => new CrmDashboardActivity(
                    x.Id.Value,
                    x.LeadId,
                    names[x.LeadId].FirstName,
                    names[x.LeadId].LastName,
            ...
            upcomingAppointmentRows
                .Select(x => new CrmDashboardAppointment(
                    x.Id,
                    x.LeadId,
                    names[x.LeadId].FirstName,
                    names[x.LeadId].LastName,
            ...
            leadRows
                ...
                .Select(x => new CrmDashboardSource(x.Key.ToString(), x.Count()))
                .ToArray(),
            leadRows
                .GroupBy(x => x.BranchId)
                .Select(x => new CrmDashboardBranchConversion(
                    x.Key,
                    x.Count(l => l.ConvertedAtUtc.HasValue),
                    x.Count()
                ))
                .ToArray(),
            inactive,
