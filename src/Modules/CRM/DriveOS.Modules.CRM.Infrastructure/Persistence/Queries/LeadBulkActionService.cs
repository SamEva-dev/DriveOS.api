...
{
    public async Task<LeadBulkActionResult> ExecuteAsync(OrganizationId org, LeadBulkActionInput input,
        CancellationToken ct)
    {
        ...
        {
            if (!byId.TryGetValue(id, out Lead? lead)) { items.Add(new(id, false, "Crm.Leads.NotFound")); continue; }
            var result = input.Action switch
            {
                LeadBulkActionType.AssignAdvisor => lead.AssignAdvisor(input.AdvisorId.HasValue
                    ? new UserId(input.AdvisorId.Value) : null),
        ...
        if (items.Any(x => x.Succeeded)) await db.SaveChangesAsync(ct);
        return new(ids.Length, items.Count(x => x.Succeeded), items.Count(x => !x.Succeeded), items);
    }
}
