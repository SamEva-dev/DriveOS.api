    ...
    : ICommandHandler<CreateBranchConfigurationOverrideDraftCommand, BranchConfigurationOverrideId>
{
        ...
        CancellationToken cancellationToken)
    {
        if (!currentUser.IsAuthenticated || currentUser.UserId is null)
            return Result.Failure<BranchConfigurationOverrideId>(BranchConfigurationOverrideErrors.CurrentUserRequired);

        var branch = await branchReadService.GetByIdAsync(
            command.OrganizationId, command.BranchId, cancellationToken);
        ...
        var baseConfiguration = await organizationConfigurationReadService.GetByIdAsync(
            command.OrganizationId, command.BaseConfigurationId, cancellationToken);

        if (baseConfiguration is null)
            return Result.Failure<BranchConfigurationOverrideId>(BranchConfigurationOverrideErrors.BaseConfigurationNotFound);

        if (baseConfiguration.Status != (int)OrganizationConfigurationStatus.Published)
            return Result.Failure<BranchConfigurationOverrideId>(BranchConfigurationOverrideErrors.BaseConfigurationMustBePublished);

        if (!string.Equals(baseConfiguration.CountryCode, command.CountryCode, StringComparison.OrdinalIgnoreCase))
            return Result.Failure<BranchConfigurationOverrideId>(BranchConfigurationOverrideErrors.CountryCodeMismatch);

        if (await repository.VersionExistsAsync(
                command.OrganizationId, command.BranchId, command.VersionNumber, cancellationToken))
            return Result.Failure<BranchConfigurationOverrideId>(BranchConfigurationOverrideErrors.VersionAlreadyExists);

        Result<BranchOverridePayload> payloadResult = BranchOverridePayload.Create(command.PayloadJson);
        if (payloadResult.IsFailure)
            return Result.Failure<BranchConfigurationOverrideId>(payloadResult.Error);

        Result<BranchConfigurationOverride> creationResult = BranchConfigurationOverride.CreateDraft(
