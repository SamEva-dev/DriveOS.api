...
) : ICommandHandler<CreateBranchCommand, BranchId>
{
    ...
    )
    {
        ...
        {
            return Result.Failure<BranchId>(BranchErrors.OrganizationNotFound);
        }
        ...
        {
            return Result.Failure<BranchId>(BranchErrors.OrganizationUnavailable);
        }
        ...
        Result<BranchCode> codeResult = BranchCode.Create(command.Code);
        ...
        {
            return Result.Failure<BranchId>(BranchErrors.DuplicateName);
        }

        if (
            await branchRepository.ExistsByCodeAsync(
                command.OrganizationId,
                codeResult.Value,
                cancellationToken
            )
        ...
        {
            return Result.Failure<BranchId>(BranchErrors.DuplicateCode);
        }
        ...
        {
            return Result.Failure<BranchId>(addressResult.Error);
        }
