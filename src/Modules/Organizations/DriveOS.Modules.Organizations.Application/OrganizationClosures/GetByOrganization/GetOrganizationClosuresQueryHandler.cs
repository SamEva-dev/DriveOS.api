...
{
    public async Task<Result<IReadOnlyList<OrganizationClosureModel>>> Handle(
        GetOrganizationClosuresQuery query,
        CancellationToken cancellationToken
    )
    {
        IReadOnlyList<OrganizationClosure> closures = await repository.ListByOrganizationAsync(
            query.OrganizationId,
            cancellationToken
        );
        return Result.Success<IReadOnlyList<OrganizationClosureModel>>(
            closures.Select(OrganizationClosureModel.FromDomain).ToArray()
        );
    }
}
