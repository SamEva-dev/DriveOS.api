...
internal sealed class ArchiveOrganizationLegalProfileCommandHandler(IOrganizationLegalProfileRepository repository,IUnitOfWork unitOfWork,ICurrentUser currentUser):ICommandHandler<ArchiveOrganizationLegalProfileCommand>
{
 public async Task<Result> Handle(ArchiveOrganizationLegalProfileCommand command,CancellationToken cancellationToken)
 {
  ...
  Result result=profile.Archive(); if(result.IsFailure)return result;
