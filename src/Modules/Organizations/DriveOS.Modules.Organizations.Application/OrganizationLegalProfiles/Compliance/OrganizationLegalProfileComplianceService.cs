...
{
    public OrganizationLegalProfileComplianceResult Validate(OrganizationLegalProfile profile)
    {
        ArgumentNullException.ThrowIfNull(profile);

        IOrganizationLegalProfileCountryRules rules = rulesProvider.GetRules(
            profile.RegisteredAddress.CountryCode
        );

        return rules.Validate(
            new OrganizationLegalProfileComplianceInput(
                profile.RegisteredAddress.CountryCode,
                profile.LegalForm,
