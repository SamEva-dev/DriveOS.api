    ...
    )
    {
        if (!currentUser.IsAuthenticated || currentUser.UserId is null)
            return Result.Failure<OrganizationLegalProfileId>(
                OrganizationLegalProfileErrors.CurrentUserRequired
            );
        ...
        if (organization.Status is "Closed" or "Archived")
            return Result.Failure<OrganizationLegalProfileId>(
                OrganizationLegalProfileErrors.OrganizationUnavailable
            );

        if (
            await repository.GetForUpdateAsync(command.OrganizationId, cancellationToken)
            is not null
        )
            return Result.Failure<OrganizationLegalProfileId>(
                OrganizationLegalProfileErrors.AlreadyExists
            );
        ...
        if (
            ...
            return Result.Failure<OrganizationLegalProfileId>(
                OrganizationLegalProfileErrors.DuplicateRegistrationNumber
            );
        ...
        Result<OrganizationLegalProfile> result = OrganizationLegalProfile.Create(
            OrganizationLegalProfileId.New(),
            command.OrganizationId,
            command.LegalForm,
        ...
        {
            OrganizationLegalProfileComplianceResult compliance = complianceService.Validate(
                result.Value
            );
