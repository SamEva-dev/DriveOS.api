...
internal sealed class UpdateOrganizationLegalProfileCommandHandler(IOrganizationReadService organizationReadService, IOrganizationLegalProfileRepository repository, IUnitOfWork unitOfWork, ICurrentUser currentUser) : ICommandHandler<UpdateOrganizationLegalProfileCommand>
{
    public async Task<Result> Handle(UpdateOrganizationLegalProfileCommand command, CancellationToken cancellationToken)
    {
        ...
        if (organization is null) return Result.Failure(OrganizationErrors.NotFound);
        if (organization.Status is "Closed" or "Archived") return Result.Failure(OrganizationLegalProfileErrors.OrganizationUnavailable);
        ...
        if (await repository.RegistrationNumberExistsAsync(countryCode, registrationNumber, command.OrganizationId, cancellationToken)) return Result.Failure(OrganizationLegalProfileErrors.DuplicateRegistrationNumber);
        ...
        Result result = profile.Update(command.LegalForm, command.RegistrationNumber, command.TaxNumber, command.TradeName, command.IncorporationDate, addressResult.Value, countryCode);
