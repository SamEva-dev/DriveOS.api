...
{
    public Task SynchronizeAsync(OrganizationRepresentative representative, CancellationToken cancellationToken = default)
    {
        ...
        return representative.Status is OrganizationRepresentativeStatus.Active
            ...
            : synchronizer.RevokeAsync(snapshot, $"Representative status is {representative.Status}.", cancellationToken);
    }

    private static OrganizationRepresentativeAccessSnapshot CreateSnapshot(OrganizationRepresentative representative) =>
        new(
            representative.Id,
