    ...
    : ICommandHandler<CreateOrganizationRepresentativeCommand, OrganizationRepresentativeId>
{
        ...
        CancellationToken cancellationToken)
    {
        if (!currentUser.IsAuthenticated || currentUser.UserId is null)
            return Result.Failure<OrganizationRepresentativeId>(OrganizationRepresentativeErrors.CurrentUserRequired);
        ...
        if (organization.Status is "Closed" or "Archived")
            return Result.Failure<OrganizationRepresentativeId>(OrganizationRepresentativeErrors.OrganizationUnavailable);

        if (await repository.ExistsActiveAsync(command.OrganizationId, command.PersonId, command.RepresentativeType, cancellationToken))
            return Result.Failure<OrganizationRepresentativeId>(OrganizationRepresentativeErrors.DuplicateActiveRepresentation);

        Result<RepresentativeAuthorityScope> scopeResult = RepresentativeAuthorityScope.Create(command.AuthorityScope);
        if (scopeResult.IsFailure)
            return Result.Failure<OrganizationRepresentativeId>(scopeResult.Error);

        if (command.IsPrimaryOwner)
        {
            OrganizationRepresentative? currentPrimary = await repository.GetPrimaryOwnerForUpdateAsync(command.OrganizationId, cancellationToken);
            if (currentPrimary is not null)
                currentPrimary.ClearPrimaryOwner();
        }

        Result<OrganizationRepresentative> result = OrganizationRepresentative.Create(
            OrganizationRepresentativeId.New(), command.OrganizationId, command.PersonId, command.UserId,
            command.RepresentativeType, scopeResult.Value, command.IsPrimaryOwner,
