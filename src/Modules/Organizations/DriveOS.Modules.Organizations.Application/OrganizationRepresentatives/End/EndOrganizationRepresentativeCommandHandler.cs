...
internal sealed class EndOrganizationRepresentativeCommandHandler(
...
) : ICommandHandler<EndOrganizationRepresentativeCommand>
{
    public async Task<Result> Handle(EndOrganizationRepresentativeCommand c, CancellationToken ct)
    {
        ...
        int remaining = e.IsOwner
            ? await repository.CountActiveOwnersAsync(c.OrganizationId, e.Id, ct)
            : 1;
        var r = e.End(
            c.EffectiveTo,
            c.Reason,
            currentUser.UserId.Value,
            e.IsOwner && remaining == 0
        );
        if (r.IsFailure)
            return r;
