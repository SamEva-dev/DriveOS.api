...
{
    public async Task<Result> Handle(UpdateOrganizationContactCommand command, CancellationToken cancellationToken)
    {
        ...
        Result<OrganizationContactInformation> valueResult = OrganizationContactInformation.Create(
            command.Email, command.Phone, command.Website);
