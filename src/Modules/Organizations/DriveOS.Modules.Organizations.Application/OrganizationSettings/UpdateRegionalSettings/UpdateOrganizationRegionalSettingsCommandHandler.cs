...
{
    public async Task<Result> Handle(UpdateOrganizationRegionalSettingsCommand command, CancellationToken cancellationToken)
    {
        ...
        Result<OrganizationRegionalSettings> valueResult = OrganizationRegionalSettings.Create(
            command.DefaultLanguage, command.SupportedLanguages, command.TimeZoneId,
            command.CurrencyCode, command.DateFormat, command.TimeFormat,
            command.FirstDayOfWeek, command.MeasurementSystem);
