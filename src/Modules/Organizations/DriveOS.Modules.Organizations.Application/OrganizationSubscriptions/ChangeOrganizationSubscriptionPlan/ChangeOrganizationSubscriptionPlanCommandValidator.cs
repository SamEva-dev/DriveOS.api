...
public sealed class ChangeOrganizationSubscriptionPlanCommandValidator
    : AbstractValidator<ChangeOrganizationSubscriptionPlanCommand>
{
    public ChangeOrganizationSubscriptionPlanCommandValidator()
    {
        ...
        RuleFor(x => x.EntitlementCodes)
            .Must(x => x.Distinct(StringComparer.Ordinal).Count() == x.Count);
        RuleFor(x => x.Limits).Must(x => x.All(i => i.Value >= 0));
    }
}
