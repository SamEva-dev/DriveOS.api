...
public sealed class CreateOrganizationSubscriptionCommandValidator : AbstractValidator<CreateOrganizationSubscriptionCommand>
{
    public CreateOrganizationSubscriptionCommandValidator()
    {
        ...
        RuleFor(x => x).Must(x => !x.CurrentPeriodEndsAtUtc.HasValue || x.CurrentPeriodEndsAtUtc > x.CurrentPeriodStartsAtUtc);
        RuleFor(x => x).Must(x => x.Status != Domain.Subscriptions.SubscriptionStatus.Trialing || (x.TrialStartsAtUtc.HasValue && x.TrialEndsAtUtc.HasValue));
