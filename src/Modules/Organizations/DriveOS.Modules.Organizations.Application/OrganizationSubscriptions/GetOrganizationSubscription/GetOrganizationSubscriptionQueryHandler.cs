...
) : IQueryHandler<GetOrganizationSubscriptionQuery, OrganizationSubscriptionResponse>
{
    public async Task<Result<OrganizationSubscriptionResponse>> Handle(
        GetOrganizationSubscriptionQuery query,
        CancellationToken cancellationToken
    )
    {
        var response = await readService.GetByOrganizationIdAsync(
            query.OrganizationId,
            cancellationToken
        );
        return response is null
            ? Result.Failure<OrganizationSubscriptionResponse>(
