    ...
    : IOrganizationConfigurationCacheInvalidator
{
    ...
    private readonly TimeSpan _ttl = TimeSpan.FromSeconds(
        Math.Clamp(
            configuration.GetValue<int?>("OrganizationConfigurations:CacheTtlSeconds") ?? 300,
            30,
            3600
        )
    );
    ...
    )
    {
        ...
        if (
            _entries.TryGetValue(key, out CacheEntry? entry)
    ...
    public void Set(
        ...
        _entries[new CacheKey(organizationId.Value, branchId?.Value)] = new CacheEntry(
            value,
    ...
    {
        foreach (
            CacheKey key in _entries.Keys.Where(key => key.OrganizationId == organizationId.Value)
        )
            _entries.TryRemove(key, out _);
    }
