...
public sealed class GenericOrganizationLegalProfileCountryRules
    : IOrganizationLegalProfileCountryRules
{
    ...
    )
    {
        ...
        if (
            input.IncorporationDate is not null
            && input.IncorporationDate > DateOnly.FromDateTime(DateTime.UtcNow)
        )
        {
            issues.Add(
                new(
