...
) : IOrganizationRepresentativeAccessSynchronizer
{
    ...
    )
    {
        ...
        {
            logger.LogDebug(
                "AuthGate representative synchronization is disabled for {RepresentativeId}.",
                representative.RepresentativeId.Value
            );
            return;
        }
        ...
        request.Headers.TryAddWithoutValidation(
            "Idempotency-Key",
            $"org-representative:{representative.RepresentativeId.Value}:r{representative.Revision}"
        );
    ...
    )
    {
        ...
        request.Headers.TryAddWithoutValidation(
            "Idempotency-Key",
            $"org-representative-revoke:{representative.RepresentativeId.Value}:r{representative.Revision}"
        );
