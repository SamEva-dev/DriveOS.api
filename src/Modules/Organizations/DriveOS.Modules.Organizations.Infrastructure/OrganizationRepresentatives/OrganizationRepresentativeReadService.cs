...
{
    public Task<OrganizationRepresentativeResponse?> GetByIdAsync(
        ...
        dbContext
            ...
            .Where(x => x.OrganizationId == organizationId && x.Id == representativeId)
            .Select(x => new OrganizationRepresentativeResponse(
                x.Id,
                x.OrganizationId,
                x.PersonId,
                x.UserId,
                x.RepresentativeType,
                x.AuthorityScope.Value,
                x.IsPrimaryOwner,
                x.EffectiveFrom,
                x.EffectiveTo,
                x.Status,
    ...
    public async Task<IReadOnlyCollection<OrganizationRepresentativeListItem>> GetListAsync(
        OrganizationId organizationId,
        OrganizationRepresentativeStatus? status,
        CancellationToken cancellationToken = default
    )
    {
        IQueryable<OrganizationRepresentative> query = dbContext
            .OrganizationRepresentatives.AsNoTracking()
            .Where(x => x.OrganizationId == organizationId);
        if (status.HasValue)
            query = query.Where(x => x.Status == status.Value);
        return await query
            .OrderByDescending(x => x.IsPrimaryOwner)
            .ThenBy(x => x.RepresentativeType)
            .ThenBy(x => x.EffectiveFrom)
            .Select(x => new OrganizationRepresentativeListItem(
                x.Id,
                x.PersonId,
                x.UserId,
                x.RepresentativeType,
