    ...
    IEntityTypeConfiguration<Branch>
{
    public void Configure(EntityTypeBuilder<Branch> builder)
    {
        ...
        builder.Property(branch => branch.Name)
            ...
            .HasConversion(
                name => name.Value,
                value => BranchName.Create(value).Value)
            .HasMaxLength(BranchName.MaximumLength)
            .IsRequired();

        builder.Property(branch => branch.NormalizedName)
        ...
        builder.Property(branch => branch.Type)
        ...
        builder.Property(branch => branch.TimeZoneId)
            .HasColumnName("time_zone_id")
            .HasMaxLength(100)
            .IsRequired();

        builder.Property(branch => branch.IsPrimary)
        ...
        builder.Property(branch => branch.CreatedByUserId)
            .HasColumnName("created_by_user_id")
            .HasConversion(
                id => id.HasValue ? id.Value.Value : (Guid?)null,
                value => value.HasValue ? new UserId(value.Value) : null);

        builder.Property(branch => branch.LastModifiedAtUtc)
            .HasColumnName("last_modified_at_utc");

        builder.Property(branch => branch.LastModifiedByUserId)
        ...
        builder.HasIndex(branch =>
                new { branch.OrganizationId, branch.Code })
            .IsUnique()
            .HasDatabaseName("ux_branches_organization_code");

        builder.HasIndex(branch =>
                new { branch.OrganizationId, branch.Status })
            .HasDatabaseName("ix_branches_organization_status");

        builder.HasIndex(branch =>
                new { branch.OrganizationId, branch.IsPrimary })
            .IsUnique()
