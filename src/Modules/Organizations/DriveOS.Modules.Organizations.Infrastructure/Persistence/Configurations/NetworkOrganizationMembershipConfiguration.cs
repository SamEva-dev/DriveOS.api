    ...
    : IEntityTypeConfiguration<NetworkOrganizationMembership>
{
    public void Configure(EntityTypeBuilder<NetworkOrganizationMembership> builder)
    {
        ...
        builder.Property(x => x.NetworkOrganizationId).HasColumnName("network_organization_id")
            .HasConversion(x => x.Value, x => new OrganizationId(x));
        builder.Property(x => x.MemberOrganizationId).HasColumnName("member_organization_id")
            .HasConversion(x => x.Value, x => new OrganizationId(x));
        ...
        builder.HasIndex(x => new { x.NetworkOrganizationId, x.MemberOrganizationId })
            .IsUnique().HasDatabaseName("ux_network_memberships_network_member");
        builder.HasIndex(x => x.MemberOrganizationId)
            .IsUnique().HasFilter("ended_at_utc IS NULL")
            .HasDatabaseName("ux_network_memberships_active_member");
        builder.HasOne<Organization>().WithMany().HasForeignKey(x => x.NetworkOrganizationId)
            .OnDelete(DeleteBehavior.Restrict);
        builder.HasOne<Organization>().WithMany().HasForeignKey(x => x.MemberOrganizationId)
            .OnDelete(DeleteBehavior.Restrict);
    }
}
