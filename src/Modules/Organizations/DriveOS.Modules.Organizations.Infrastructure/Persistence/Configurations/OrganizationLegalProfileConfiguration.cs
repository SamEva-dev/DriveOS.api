    ...
    IEntityTypeConfiguration<OrganizationLegalProfile>
{
    public void Configure(EntityTypeBuilder<OrganizationLegalProfile> builder)
    {
        ...
        builder.OwnsOne(x => x.RegisteredAddress, address =>
        {
            address.Property(x => x.Line1).HasColumnName("registered_address_line1").HasMaxLength(200).IsRequired();
            address.Property(x => x.Line2).HasColumnName("registered_address_line2").HasMaxLength(200);
            address.Property(x => x.PostalCode).HasColumnName("registered_postal_code").HasMaxLength(30).IsRequired();
            address.Property(x => x.City).HasColumnName("registered_city").HasMaxLength(120).IsRequired();
            address.Property(x => x.Region).HasColumnName("registered_region").HasMaxLength(120);
            address.Property(x => x.CountryCode).HasColumnName("registered_country_code").HasMaxLength(2).IsFixedLength().IsRequired();
        });

        builder.Property(x => x.CreatedAtUtc).HasColumnName("created_at_utc").IsRequired();
        builder.Property(x => x.CreatedByUserId)
            .HasColumnName("created_by_user_id")
            .HasConversion(v => v.HasValue ? v.Value.Value : (Guid?)null, v => v.HasValue ? new UserId(v.Value) : null);
        ...
        builder.HasIndex(x => new { x.RegistrationNumber, x.Status })
