    ...
    IEntityTypeConfiguration<OrganizationSubscription>
{
    public void Configure(EntityTypeBuilder<OrganizationSubscription> builder)
    {
        ...
        builder.OwnsOne(subscription => subscription.CurrentPeriod, period =>
        {
            period.Property(value => value.StartsAtUtc)
                .HasColumnName("current_period_starts_at_utc")
                .IsRequired();
            period.Property(value => value.EndsAtUtc)
                .HasColumnName("current_period_ends_at_utc");
        });

        builder.OwnsOne(subscription => subscription.TrialPeriod, period =>
        {
            period.Property(value => value.StartsAtUtc)
        ...
        builder.OwnsOne(subscription => subscription.Cancellation, cancellation =>
        {
            cancellation.Property(value => value.RequestedAtUtc)
                .HasColumnName("cancellation_requested_at_utc");
            cancellation.Property(value => value.EffectiveAtUtc)
                .HasColumnName("cancellation_effective_at_utc");
            cancellation.Property(value => value.Reason)
                .HasColumnName("cancellation_reason")
                .HasMaxLength(SubscriptionCancellation.ReasonMaximumLength);
            cancellation.Property(value => value.RequestedByUserId)
                .HasColumnName("cancellation_requested_by_user_id")
                .HasConversion(id => id.Value, value => new UserId(value));
        });

        builder.Property(subscription => subscription.ExternalProvider)
        ...
        builder.Property(subscription => subscription.LastModifiedAtUtc)
            .HasColumnName("last_modified_at_utc");

        builder.Property(subscription => subscription.LastModifiedByUserId)
        ...
        builder.OwnsMany(subscription => subscription.Entitlements, entitlement =>
        {
            entitlement.ToTable("organization_subscription_entitlements");

            entitlement.WithOwner()
                .HasForeignKey(subscriptionForeignKey);

            entitlement.Property<OrganizationSubscriptionId>(subscriptionForeignKey)
                .HasColumnName("organization_subscription_id")
                .HasConversion(
                    id => id.Value,
                    value => new OrganizationSubscriptionId(value));

            entitlement.Property(value => value.Code)
                .HasColumnName("entitlement_code")
                .HasMaxLength(SubscriptionEntitlement.CodeMaximumLength)
                .IsRequired();

            entitlement.HasKey(
                subscriptionForeignKey,
                nameof(SubscriptionEntitlement.Code));
        });

        builder.OwnsMany(subscription => subscription.Limits, limit =>
        {
            ...
            limit.Property<OrganizationSubscriptionId>(subscriptionForeignKey)
                .HasColumnName("organization_subscription_id")
                .HasConversion(
                    id => id.Value,
                    value => new OrganizationSubscriptionId(value));

            limit.Property(value => value.Code)
                .HasColumnName("limit_code")
                .HasMaxLength(SubscriptionLimit.CodeMaximumLength)
                .IsRequired();

            limit.Property(value => value.Value)
                .HasColumnName("limit_value")
                .IsRequired();

            limit.HasKey(
                subscriptionForeignKey,
                nameof(SubscriptionLimit.Code));
        });
