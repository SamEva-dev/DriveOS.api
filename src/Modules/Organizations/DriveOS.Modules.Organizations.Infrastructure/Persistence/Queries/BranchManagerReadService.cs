...
internal sealed class BranchManagerReadService(OrganizationsDbContext dbContext)
    : IBranchManagerReadService
{
    public Task<BranchManagerAssignmentItem?> GetCurrentAsync(
    ...
    {
        return (
            from assignment in dbContext.BranchManagerAssignments.AsNoTracking()

            join branch in dbContext.Branches.AsNoTracking() on assignment.BranchId equals branch.Id
            ...
            where
                ...
                && assignment.EffectiveFromUtc <= atUtc
                && (assignment.EffectiveToUtc == null || assignment.EffectiveToUtc > atUtc)

            orderby assignment.EffectiveFromUtc descending

            select new BranchManagerAssignmentItem(
                assignment.Id.Value,
    ...
    public async Task<IReadOnlyList<BranchManagerAssignmentItem>> GetHistoryAsync(
        OrganizationId organizationId,
        BranchId branchId,
        CancellationToken cancellationToken = default
    )
    {
        return await (
            ...
            where branch.Id == branchId && branch.OrganizationId == organizationId
