...
internal sealed class OrganizationSubscriptionReadService(OrganizationsDbContext dbContext)
    : IOrganizationSubscriptionReadService
{
    public async Task<OrganizationSubscriptionResponse?> GetByOrganizationIdAsync(
    ...
    )
    {
        ...
        return new OrganizationSubscriptionResponse(
            ...
            (int)subscription.BillingCycle,
            new SubscriptionPeriodResponse(
                subscription.CurrentPeriod.StartsAtUtc,
                subscription.CurrentPeriod.EndsAtUtc
            ),
            subscription.TrialPeriod is null
                ? null
                : new SubscriptionPeriodResponse(
                    subscription.TrialPeriod.StartsAtUtc,
                    subscription.TrialPeriod.EndsAtUtc
                ),
            subscription.Cancellation is null
                ? null
                : new SubscriptionCancellationResponse(
                    subscription.Cancellation.RequestedAtUtc,
                    subscription.Cancellation.EffectiveAtUtc,
                    subscription.Cancellation.Reason,
                    subscription.Cancellation.RequestedByUserId.Value
                ),
            subscription.ExternalProvider,
            subscription.ExternalSubscriptionId,
            subscription
                .Entitlements.Select(x => new SubscriptionEntitlementResponse(x.Code))
                .ToArray(),
            subscription
                .Limits.Select(x => new SubscriptionLimitResponse(x.Code, x.Value))
                .ToArray(),
            subscription.Version,
            subscription.CreatedAtUtc,
            subscription.LastModifiedAtUtc
        );
    }
    ...
    public Task<bool> HasEntitlementAsync(
        ...
        dbContext
            ...
            .AnyAsync(x => x.Code == entitlementCode.Trim(), cancellationToken);
