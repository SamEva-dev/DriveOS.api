...
public sealed class CrmActivityTests
{
    ...
    {
        var result = CrmActivity.Create(
            CrmActivityId.New(),
            OrganizationId,
            LeadId,
            CrmActivityType.Call,
            CrmActivityDirection.Outbound,
            "  Appel de suivi  ",
    ...
    {
        CrmActivity activity = CrmActivity
            .Create(
                CrmActivityId.New(),
                OrganizationId,
                LeadId,
        ...
        var result = activity.Invalidate(
            "Activité créée par erreur",
            userId,
            DateTimeOffset.UtcNow
        );
        Assert.True(result.IsSuccess);
    ...
    {
        var metadata = new CrmActivityMetadata(
            null,
            null,
            false,
            false,
            ...
            CrmActivitySyncStatus.Failed,
            "ext-1",
            "key-1",
            "Crm.Sync.LeadNotFound",
            1,
            DateTimeOffset.UtcNow,
            null,
            null
    ...
    public void Invalidate_ShouldRejectASecondInvalidation()
    {
        ...
        Assert.True(
            activity.Invalidate("Erreur de saisie", userId, DateTimeOffset.UtcNow).IsSuccess
        );
