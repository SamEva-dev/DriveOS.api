...
public sealed class ImportCrmActivityCommandHandlerTests
{
    ...
    public async Task ExistingKey_ShouldReturnExistingActivityAfterAcquiringTheLock()
    {
        ...
        CrmActivity existing = CrmActivity.Create(CrmActivityId.New(), organizationId, null,
            CrmActivityType.Email, CrmActivityDirection.Inbound, "Import", null,
            DateTimeOffset.UtcNow, null, CrmActivityMetadata.Imported("ext", "key",
                CrmActivitySyncStatus.Synchronized, DateTimeOffset.UtcNow)).Value;
        var repository = new FakeActivityRepository(existing);
        var unitOfWork = new FakeUnitOfWork();
        var importLock = new FakeImportLock(unitOfWork);
        var handler = new ImportCrmActivityCommandHandler(new FakeLeadRepository(), repository,
            importLock, unitOfWork, new FakeClock(DateTimeOffset.UtcNow));
    ...
    private static ImportCrmActivityCommand Command(OrganizationId organizationId) => new(
        organizationId, null, CrmActivityType.Email, CrmActivityDirection.Inbound,
        "Import", null, DateTimeOffset.UtcNow, null, "ext", "key",
        CrmActivitySyncStatus.Synchronized, null, null, null, false, null, null);
    ...
    private sealed class FakeImportLock(FakeUnitOfWork unitOfWork) : ICrmActivityImportLock
    {
        ...
        public Task AcquireAsync(OrganizationId organizationId, string idempotencyKey, CancellationToken ct)
        {
    ...
    private sealed class FakeActivityRepository(CrmActivity existing) : ICrmActivityRepository
    {
        ...
        public Task<CrmActivity?> GetByIdempotencyKeyAsync(OrganizationId organizationId,
            string idempotencyKey, CancellationToken cancellationToken = default) => Task.FromResult<CrmActivity?>(existing);
        public Task<IReadOnlyList<CrmActivity>> GetByLeadAsync(OrganizationId organizationId,
            LeadId leadId, CancellationToken cancellationToken = default) => throw new NotSupportedException();
    ...
    private sealed class FakeUnitOfWork : ICrmUnitOfWork
    {
        ...
        { HasActiveTransaction = true; return Task.CompletedTask; }
        public Task RollbackTransactionAsync(CancellationToken cancellationToken = default)
        { HasActiveTransaction = false; RollbackCalled = true; return Task.CompletedTask; }
        public Task CommitTransactionAsync(CancellationToken cancellationToken = default)
        { HasActiveTransaction = false; return Task.CompletedTask; }
    ...
    {
        public Task<Lead?> GetByIdAsync(OrganizationId organizationId, LeadId id,
            CancellationToken cancellationToken = default) => Task.FromResult<Lead?>(null);
        public Task<Lead?> GetByIdForUpdateAsync(OrganizationId organizationId, LeadId id,
            CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task<bool> ExistsByEmailAsync(OrganizationId organizationId, string email,
            CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task AddAsync(Lead lead, CancellationToken cancellationToken = default) => throw new NotSupportedException();
    }
}
