...
public sealed class GetCrmDashboardQueryHandlerTests
{
    ...
    public async Task Handle_OrganizationScope_ForwardsTenantAndClock()
    {
        ...
        var result = await handler.Handle(new GetCrmDashboardQuery(
            [organizationId], "organization", Guid.NewGuid(), EmptyFilters()), CancellationToken.None);
    ...
    {
        var service = new FakeDashboardReadService();
        var handler = new GetCrmDashboardQueryHandler(service,
            new FakeClock(DateTimeOffset.UtcNow));
        Guid branchId = Guid.NewGuid();

        await handler.Handle(new GetCrmDashboardQuery(
            [OrganizationId.New()], "branch", branchId, EmptyFilters()), CancellationToken.None);
    ...
    private sealed class FakeDashboardReadService : ICrmDashboardReadService
    {
        ...
        public Task<CrmDashboardResponse> GetAsync(IReadOnlyCollection<OrganizationId> organizationIds,
            ...
            CancellationToken cancellationToken)
        {
            ...
            NowUtc = nowUtc;
            Filters = filters;
            return Task.FromResult(new CrmDashboardResponse(nowUtc,
                branchId.HasValue ? "Branch" : "Organization", branchId,
