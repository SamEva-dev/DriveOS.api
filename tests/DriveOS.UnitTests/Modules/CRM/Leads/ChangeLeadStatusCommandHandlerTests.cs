...
public sealed class ChangeLeadStatusCommandHandlerTests
{
    ...
    public async Task Handle_WithAllowedTransition_ChangesStatusAndCommits()
    {
        ...
        Result result = await handler.Handle(
            new ChangeLeadStatusCommand(organizationId, lead.Id, LeadStatus.Contacted, null),
    ...
    {
        var unitOfWork = new FakeCrmUnitOfWork();
        var handler = new ChangeLeadStatusCommandHandler(new FakeLeadRepository(null), unitOfWork);
    ...
    private static Lead CreateLead(OrganizationId organizationId) =>
        Lead.Create(
            ...
            LeadSource.Create(LeadSourceType.PhoneCall).Value
        ).Value;
