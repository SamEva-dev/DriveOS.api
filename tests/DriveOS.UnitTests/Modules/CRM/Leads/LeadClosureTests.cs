...
public sealed class LeadClosureTests
{
    ...
    public void Close_PreservesLeadAndStopsAutomaticFollowUps()
    {
        ...
        var result = lead.Close(LeadStatus.Lost, LeadClosureReason.PriceTooHigh,
            "Budget insuffisant", DateTimeOffset.Parse("2026-08-12T12:00:00Z"));
    ...
    {
        Lead lead = CreateLead(); var now = DateTimeOffset.Parse("2026-08-12T12:00:00Z");
        var result = lead.SetDormant(LeadClosureReason.ProjectPostponed, now,
            UserId.New(), null, null, now);
    ...
    {
        Lead lead = CreateLead();
        var result = lead.ReferToPartner("Partenaire A", "Nom et téléphone", default,
            null, DateTimeOffset.Parse("2026-08-12T12:00:00Z"));
        result.Error.Code.Should().Be("Crm.Leads.Referral.Consent.Required");
    }

    private static Lead CreateLead() => Lead.Create(LeadId.New(), OrganizationId.New(), null,
