...
public sealed class CommercialOfferTests
{
    ...
    {
        var result = CreateOffer([new(OfferLineType.PracticalLesson, null, "20 heures",
            20, "hour", 50, 100, 20, true, OfferPriceSource.StandardCatalog, null)], 800);
    ...
    {
        var result = CreateOffer([new(OfferLineType.Other, null, "Prix négocié",
            1, "package", 900, 0, 0, true, OfferPriceSource.ManualOverride, null)], 0);
    ...
    {
        CommercialOffer offer = CreateOffer([new(OfferLineType.TheoryTraining, null,
            "Code en ligne", 1, "package", 200, 0, 20, true,
            OfferPriceSource.StandardCatalog, null)], 0).Value;

        offer.PrepareDelivery(OfferDeliveryChannel.Email, "[{\"type\":\"Prospect\"}]",
            "Subject", "Message", "fr", "offer.pdf", "[]", new string('a', 64),
            Now.AddHours(24), Now).IsFailure.Should().BeTrue();
        offer.SubmitForReview().IsSuccess.Should().BeTrue();
        offer.Approve().IsSuccess.Should().BeTrue();
        offer.PrepareDelivery(OfferDeliveryChannel.Email, "[{\"type\":\"Prospect\"}]",
            "Subject", "Message", "fr", "offer.pdf", "[]", new string('a', 64),
            Now.AddHours(24), Now).IsSuccess.Should().BeTrue();
        offer.MarkSent(Now).IsSuccess.Should().BeTrue();
    }

    private static DriveOS.SharedKernel.Results.Result<CommercialOffer> CreateOffer(
        IReadOnlyCollection<CommercialOfferLineDraft> lines, decimal funding) =>
        CommercialOffer.Generate(CommercialOfferId.New(), new OrganizationId(Guid.NewGuid()),
            new LeadId(Guid.NewGuid()), AssessmentSessionId.New(), 4, null, 1,
            "FR-PERMIS-B", "eur", Now.AddDays(15), Now, funding,
            "Estimated only", "Valid 15 days", null, lines);
}
