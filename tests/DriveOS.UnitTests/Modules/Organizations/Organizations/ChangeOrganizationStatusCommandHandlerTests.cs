...
{
    private static readonly OrganizationId OrganizationId = new(
        Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa")
    );

    private static readonly UserId CurrentUserId = new(
        Guid.Parse("bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb")
    );

    private static readonly DateTimeOffset UtcNow = new(2026, 7, 29, 8, 30, 0, TimeSpan.Zero);

    [Fact]
    public async Task Handle_WhenDraftIsSubmittedForActivation_ShouldSucceed()
    {
        // Arrange
        Organization organization = OrganizationTestData.CreateDraft(OrganizationId);

        var repository = new FakeOrganizationRepository { Organization = organization };

        var unitOfWork = new FakeUnitOfWork();

        var currentUser = FakeCurrentUser.Authenticated(CurrentUserId);

        var clock = new FakeClock(UtcNow);

        var handler = new ChangeOrganizationStatusCommandHandler(
            repository,
        ...
        var command = new ChangeOrganizationStatusCommand(
        ...
        Assert.Equal(OrganizationStatus.PendingActivation, organization.Status);

        Assert.Equal(1, unitOfWork.CommitCallCount);

        Assert.Equal(false, repository.LastAsNoTracking);

        Assert.Equal(OrganizationId, repository.LastRequestedOrganizationId);

        OrganizationStatusHistoryEntry historyEntry = Assert.Single(organization.StatusHistory);

        Assert.Equal(OrganizationStatus.Draft, historyEntry.PreviousStatus);

        Assert.Equal(OrganizationStatus.PendingActivation, historyEntry.NewStatus);

        Assert.Equal(CurrentUserId.Value, historyEntry.ChangedByUserId);

        Assert.Equal(UtcNow, historyEntry.ChangedAtUtc);

        Assert.Equal("Dossier soumis pour vérification.", historyEntry.Reason.Value);
    }
    ...
    {
        // Arrange
        Organization organization = OrganizationTestData.CreatePendingActivation(OrganizationId);

        int initialHistoryCount = organization.StatusHistory.Count;
        ...
        Assert.Equal(initialHistoryCount + 1, organization.StatusHistory.Count);

        OrganizationStatusHistoryEntry lastEntry = organization.StatusHistory.Last();

        Assert.Equal(OrganizationStatus.PendingActivation, lastEntry.PreviousStatus);

        Assert.Equal(OrganizationStatus.Active, lastEntry.NewStatus);

        Assert.Equal(CurrentUserId.Value, lastEntry.ChangedByUserId);

        Assert.Equal(UtcNow, lastEntry.ChangedAtUtc);
    ...
    {
        // Arrange
        Organization organization = OrganizationTestData.CreateActive(OrganizationId);
        ...
        Assert.Equal(OrganizationStatus.Restricted, organization.Status);

        OrganizationStatusHistoryEntry lastEntry = organization.StatusHistory.Last();

        Assert.Equal(OrganizationStatus.Active, lastEntry.PreviousStatus);

        Assert.Equal(OrganizationStatus.Restricted, lastEntry.NewStatus);
    ...
    {
        // Arrange
        Organization organization = OrganizationTestData.CreateRestricted(OrganizationId);
        ...
        Assert.Equal(OrganizationStatus.Restricted, lastEntry.PreviousStatus);

        Assert.Equal(OrganizationStatus.Suspended, lastEntry.NewStatus);
    ...
    {
        // Arrange
        Organization organization = OrganizationTestData.CreateSuspended(OrganizationId);
        ...
        Assert.Equal(OrganizationStatus.Suspended, lastEntry.PreviousStatus);
    ...
    public async Task Handle_WhenActiveOrganizationIsClosed_ShouldSucceed()
    {
        ...
        Assert.Equal(OrganizationStatus.Closed, lastEntry.NewStatus);
    ...
    {
        // Arrange
        var repository = new FakeOrganizationRepository { Organization = null };
        ...
        Assert.Equal("Organizations.NotFound", result.Error.Code);
    ...
    public async Task Handle_WhenTransitionIsInvalid_ShouldReturnConflict()
    {
        ...
        Assert.Equal("Organizations.InvalidStatusTransition", result.Error.Code);

        Assert.Equal(OrganizationStatus.Draft, organization.Status);

        Assert.Empty(organization.StatusHistory);
    ...
    {
        // Arrange
        Organization organization = OrganizationTestData.CreateClosed(OrganizationId);
        ...
        Assert.Equal(initialHistoryCount, organization.StatusHistory.Count);
    ...
    public async Task Handle_ShouldUseCurrentAuthenticatedUser()
    {
        ...
        UserId expectedUserId = new(Guid.Parse("cccccccc-cccc-cccc-cccc-cccccccccccc"));

        var repository = new FakeOrganizationRepository { Organization = organization };

        var unitOfWork = new FakeUnitOfWork();

        var currentUser = FakeCurrentUser.Authenticated(expectedUserId);
        ...
        OrganizationStatusHistoryEntry entry = Assert.Single(organization.StatusHistory);

        Assert.Equal(expectedUserId.Value, entry.ChangedByUserId);
    }
    ...
    public async Task Handle_ShouldUseInjectedClock()
    {
        ...
        DateTimeOffset expectedDate = new(2030, 1, 15, 14, 45, 0, TimeSpan.Zero);
        ...
        var clock = new FakeClock(expectedDate);
        ...
        Assert.Equal(expectedDate, entry.ChangedAtUtc);
    }
    ...
    public async Task Handle_WhenCurrentUserIsNotAuthenticated_ShouldFail()
    {
        ...
        var currentUser = FakeCurrentUser.Anonymous();
        ...
        Assert.Equal("Organizations.CurrentUser.Required", result.Error.Code);
    ...
    private static ChangeOrganizationStatusCommandHandler CreateHandler(
    ...
    )
    {
        return new ChangeOrganizationStatusCommandHandler(
            ...
            new FakeActivationReadinessAuditSink(),
            unitOfWork,
            FakeCurrentUser.Authenticated(CurrentUserId),
    ...
    private sealed class FakeOrganizationRepository : IOrganizationRepository
    {
        ...
        public OrganizationId? LastRequestedOrganizationId { get; private set; }
        ...
        {
            throw new NotSupportedException("AddAsync is not used by lifecycle tests.");
        }

        public void Update(Organization entity)
        {
            throw new NotSupportedException("Update is not used by lifecycle tests.");
        }

        public void Remove(Organization entity)
        {
            throw new NotSupportedException("Remove is not used by lifecycle tests.");
        }
    }
    ...
    private sealed class FakeCurrentUser : ICurrentUser
    {
        ...
        private FakeCurrentUser(bool isAuthenticated, UserId? userId, string? email)
        {
        ...
        public IReadOnlySet<string> Permissions => EmptyPermissions;
        ...
        public static FakeCurrentUser Authenticated(UserId userId)
        {
            return new FakeCurrentUser(true, userId, "admin@driveos.test");
        }
