...
internal sealed class FakeOrganizationReadService : IOrganizationReadService
{
    public OrganizationResponse? OrganizationByIdResult { get; set; }

    public PagedResult<OrganizationListItem> PagedResult { get; set; } =
        new([], PageNumber: 1, PageSize: 20, TotalCount: 0);

    public IReadOnlyList<OrganizationStatusHistoryItem> StatusHistoryResult { get; set; } = [];
    ...
    public OrganizationSortField? LastSortBy { get; private set; }
    ...
    )
    {
        ...
        return Task.FromResult(OrganizationByIdResult);
    }
