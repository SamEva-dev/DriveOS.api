...
public sealed class GetOrganizationsQueryHandlerTests
{
    ...
    public async Task Handle_ShouldReturnPagedOrganizations()
    {
        ...
        var expectedPage =
            new PagedResult<OrganizationListItem>(
                [expectedItem],
                PageNumber: 2,
                PageSize: 10,
                TotalCount: 21);
        ...
        var readService =
            ...
            {
                PagedResult = expectedPage
            };

        var handler =
            new GetOrganizationsQueryHandler(
                readService);
        ...
        Assert.Equal(
            2,
            readService.LastPageNumber);

        Assert.Equal(
            10,
            readService.LastPageSize);

        Assert.Equal(
            "horizon",
            readService.LastSearch);

        Assert.Equal(
            OrganizationSortField.CreatedAtUtc,
            readService.LastSortBy);

        Assert.Equal(
            SortDirection.Descending,
            readService.LastSortDirection);
    }
}